...
proptest! {
  ...
  fn blake2b_fixed_outputs_match_rustcrypto(
    ...
    prop_assert_eq!(&Blake2b256::keyed_hash(key, &data)[..], &expected_keyed_256[..]);
    ...
    prop_assert_eq!(&Blake2b512::keyed_hash(key, &data)[..], &expected_keyed_512[..]);
  ...
  fn blake2s_fixed_outputs_match_rustcrypto(
    ...
    prop_assert_eq!(&Blake2s128::keyed_hash(key, &data)[..], &expected_keyed_128[..]);

    let mut oracle_keyed_256 = OracleBlake2sMac256::new_from_slice(key).unwrap();
    oracle_keyed_256.update(&data);
    let expected_keyed_256 = oracle_keyed_256.finalize().into_bytes();
    prop_assert_eq!(&Blake2s256::keyed_hash(key, &data)[..], &expected_keyed_256[..]);
