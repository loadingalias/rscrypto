//! Integration oracle tests for AES-128-GCM.
//!
//! Validates rscrypto's AES-128-GCM against the RustCrypto `aes-gcm` crate
...
use rscrypto::{
  Aes128Gcm, Aes128GcmKey, Aes128GcmTag,
...
fn assert_matches_oracle(key_bytes: &[u8; 16], nonce_bytes: &[u8; 12], aad: &[u8], plaintext: &[u8]) {
  let key = Aes128GcmKey::from_bytes(*key_bytes);
  ...
  let tag = cipher
    ...
    .expect("rscrypto must seal valid AES-128-GCM oracle input");
  ...
  let oracle_tag = oracle
    ...
    .expect("RustCrypto must seal valid AES-128-GCM oracle input");
  ...
  cipher
    ...
    .expect("fresh AES-128-GCM ciphertext must authenticate");
...
#[test]
fn aes128gcm_matches_rustcrypto_oracle() {
  let key = [0x42u8; 16];
  let nonce = [0x24u8; 12];
  let aad = b"rscrypto-aes128-gcm-oracle";
...
#[test]
fn aes128gcm_oracle_large_input() {
  let key = [0x77u8; 16];
  let nonce = [0x88u8; 12];
...
#[test]
fn aes128gcm_oracle_aad_size_sweep() {
  let key = [0x99u8; 16];
  let nonce = [0xAAu8; 12];
...
#[test]
fn aes128gcm_oracle_all_short_lengths() {
  let key = [0xA8u8; 16];

  for size in 0..=255usize {
    let mut nonce = [0x18u8; 12];
    ...
    let aad = if size % 3 == 0 {
      ...
      deterministic_bytes(0x42 ^ size_byte, size % 97)
    };
    let plaintext = deterministic_bytes(0x81 ^ size_byte, size);
    assert_matches_oracle(&key, &nonce, &aad, &plaintext);
  }
}

#[test]
fn aes128gcm_oracle_large_aligned_and_unaligned_lengths() {
  let key = [0x5Du8; 16];
  let aad_cases = [Vec::new(), deterministic_bytes(0xAC, 257)];

  for (case_idx, &size) in [4096usize, 4097, 8191, 8192, 8193, 16_384, 16_385].iter().enumerate() {
    let mut nonce = [0x71u8; 12];
    ...
    nonce[8..12].copy_from_slice(&size_word.to_be_bytes());
    let plaintext = deterministic_bytes(0x33 ^ case_idx.to_le_bytes()[0], size);
...
#[test]
fn aes128gcm_rejects_modified_tag() {
  let key = Aes128GcmKey::from_bytes([0x11; 16]);
  ...
  let mut tag = cipher
    ...
    .expect("AES-128-GCM tag-forgery fixture must seal")
    .to_bytes();
  tag[0] ^= 1;

  cipher
    .decrypt_in_place(&nonce, b"aad", &mut buffer, &Aes128GcmTag::from_bytes(tag))
    .expect_err("AES-128-GCM must reject a modified tag");
}

#[test]
fn aes128gcm_rejects_modified_ciphertext() {
  let key = Aes128GcmKey::from_bytes([0x33; 16]);
  ...
  let tag = cipher
    ...
    .expect("AES-128-GCM ciphertext-tampering fixture must seal");
  buffer[0] ^= 1;

  cipher
    .decrypt_in_place(&nonce, b"", &mut buffer, &tag)
    .expect_err("AES-128-GCM must reject modified ciphertext");
}

#[test]
fn aes128gcm_rejects_wrong_aad() {
  let key = Aes128GcmKey::from_bytes([0x55; 16]);
  ...
  let tag = cipher
    ...
    .expect("AES-128-GCM AAD-mismatch fixture must seal");

  cipher
    .decrypt_in_place(&nonce, b"wrong", &mut buffer, &tag)
    .expect_err("AES-128-GCM must reject incorrect associated data");
}
