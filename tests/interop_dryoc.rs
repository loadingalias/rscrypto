...
const DATA: &[u8] = b"dryoc interoperability equivalence data";
const KEY_32: [u8; 32] = [0x42; 32];
const KEY_64: [u8; 64] = [0x24; 64];

#[test]
fn test_dryoc_blake2b_are_byte_equivalent() {
  let mut dryoc_b256 = [0u8; 32];
  crypto_generichash(&mut dryoc_b256, DATA, None).expect("dryoc BLAKE2b-256 interoperability hash must succeed");
  assert_eq!(Blake2b256::digest(DATA), dryoc_b256);

  let mut dryoc_b512 = [0u8; 64];
  crypto_generichash(&mut dryoc_b512, DATA, None).expect("dryoc BLAKE2b-512 interoperability hash must succeed");
  ...
  crypto_generichash(&mut dryoc_keyed_b256, DATA, Some(&KEY_32))
    .expect("dryoc keyed BLAKE2b-256 interoperability hash must succeed");
  assert_eq!(
    Blake2b256::keyed_digest(
      Blake2bKey::new(&KEY_32).expect("32-byte BLAKE2b interoperability key must be valid"),
      DATA,
    ),
    dryoc_keyed_b256
  );
  ...
  crypto_generichash(&mut dryoc_keyed_b512, DATA, Some(&KEY_64))
    .expect("dryoc keyed BLAKE2b-512 interoperability hash must succeed");
  assert_eq!(
    Blake2b512::keyed_digest(
      Blake2bKey::new(&KEY_64).expect("64-byte BLAKE2b interoperability key must be valid"),
      DATA,
    ),
    dryoc_keyed_b512
  );
...
fn test_dryoc_ed25519_are_byte_equivalent() {
  ...
  crypto_sign_detached(&mut dryoc_signature, DATA, &dryoc_secret)
    .expect("dryoc Ed25519 interoperability signing must succeed");
  ...
  crypto_sign_verify_detached(ours_signature.as_bytes(), DATA, &dryoc_public)
    .expect("dryoc must verify the rscrypto Ed25519 interoperability signature");
  ours_public
...
#[test]
fn test_dryoc_x25519_are_byte_equivalent() {
  ...
  let ours_shared = ours_alice
    ...
    .expect("rscrypto X25519 interoperability exchange must produce a nonzero secret");
  ...
  crypto_scalarmult(&mut dryoc_shared, &alice_bytes, &dryoc_bob_public)
    .expect("dryoc X25519 interoperability exchange must produce a nonzero secret");
