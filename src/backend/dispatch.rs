...
///
/// ```text
/// use rscrypto::backend::{candidates, caps::Caps};
///
...
macro_rules! candidates {
  ...
  [ $( $name:literal => $caps:expr => $func:expr ),+ $(,)? ] => {
    &[
      $(
        $crate::backend::dispatch::Candidate::new($name, $caps, $func as _),
      )+
    ]
  };
}
...
macro_rules! define_dispatcher {
  (
    ...
    pub struct $name {
      ...
      cache: $crate::backend::cache::OnceCache<$crate::backend::dispatch::Selected<$fn_ty>>,
      /// Selector function called on first access.
      selector: fn() -> $crate::backend::dispatch::Selected<$fn_ty>,
    }

    impl $name {
      /// Create a new dispatcher with the given selector function.
      #[must_use]
      pub const fn new(selector: fn() -> $crate::backend::dispatch::Selected<$fn_ty>) -> Self {
        Self {
          cache: $crate::backend::cache::OnceCache::new(),
          selector,
        }
      }
      ...
      #[inline]
      #[must_use]
      pub fn get(&self) -> $crate::backend::dispatch::Selected<$fn_ty> {
