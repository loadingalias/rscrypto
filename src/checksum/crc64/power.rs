...
impl Simd {
  ...
  unsafe fn to_le(self) -> Self {
    ...
    {
      // SAFETY: POWER8 vector byte-swap is available under this function's target-feature contract.
      unsafe {
        let out: i64x2;
        asm!(
          "xxbrd {out}, {inp}",
  ...
  unsafe fn fold_16(self, coeff: Self) -> Self {
    // SAFETY: POWER8 vector crypto intrinsics/asm are available via this function's #[target_feature]
    // attribute.
    unsafe { Self(Self::vpmsumd(self.0, coeff.0)) }
  }
...
unsafe fn crc64_vpmsum(mut state: u64, bytes: &[u8], consts: &Crc64ClmulConstants, tables: &[[u64; 256]; 16]) -> u64 {
  // SAFETY: POWER8 vector crypto intrinsics/asm are available via this function's #[target_feature]
  // attribute. align_to is sound because Simd is repr(transparent) over i64x2.
  unsafe {
...
/// `crate::platform::caps().has(crate::platform::caps::power::VPMSUM_READY)`.
#[target_feature(
  enable = "altivec",
  enable = "vsx",
  enable = "power8-vector",
...
pub(crate) unsafe fn crc64_xz_vpmsum(crc: u64, data: &[u8]) -> u64 {
  // SAFETY: POWER8 vector crypto intrinsics/asm are available via this function's #[target_feature]
  // attribute.
  unsafe {
    crc64_vpmsum(
...
pub(crate) unsafe fn crc64_xz_vpmsum_2way(crc: u64, data: &[u8]) -> u64 {
  // SAFETY: POWER8 vector crypto intrinsics/asm are available via this function's #[target_feature]
  // attribute.
  unsafe {
    crc64_vpmsum_2way(
...
pub(crate) unsafe fn crc64_xz_vpmsum_4way(crc: u64, data: &[u8]) -> u64 {
  // SAFETY: POWER8 vector crypto intrinsics/asm are available via this function's #[target_feature]
  // attribute.
  unsafe {
    crc64_vpmsum_4way(
...
pub(crate) unsafe fn crc64_xz_vpmsum_8way(crc: u64, data: &[u8]) -> u64 {
  // SAFETY: POWER8 vector crypto intrinsics/asm are available via this function's #[target_feature]
  // attribute.
  unsafe {
    crc64_vpmsum_8way(
...
pub(crate) unsafe fn crc64_nvme_vpmsum(crc: u64, data: &[u8]) -> u64 {
  // SAFETY: POWER8 vector crypto intrinsics/asm are available via this function's #[target_feature]
  // attribute.
  unsafe {
    crc64_vpmsum(
...
pub(crate) unsafe fn crc64_nvme_vpmsum_2way(crc: u64, data: &[u8]) -> u64 {
  // SAFETY: POWER8 vector crypto intrinsics/asm are available via this function's #[target_feature]
  // attribute.
  unsafe {
    crc64_vpmsum_2way(
...
pub(crate) unsafe fn crc64_nvme_vpmsum_4way(crc: u64, data: &[u8]) -> u64 {
  // SAFETY: POWER8 vector crypto intrinsics/asm are available via this function's #[target_feature]
  // attribute.
  unsafe {
    crc64_vpmsum_4way(
...
pub(crate) unsafe fn crc64_nvme_vpmsum_8way(crc: u64, data: &[u8]) -> u64 {
  // SAFETY: POWER8 vector crypto intrinsics/asm are available via this function's #[target_feature]
  // attribute.
  unsafe {
    crc64_vpmsum_8way(
...
mod tests {
  ...
  #[test]
  fn vpmsum_kernels_match_portable() {
    if !crate::platform::caps().has(crate::platform::caps::power::VPMSUM_READY) {
      return;
    }

    for (name, kernel) in [
      ("xz/vpmsum", crc64_xz_vpmsum_safe as fn(u64, &[u8]) -> u64),
      ("xz/vpmsum-2way", crc64_xz_vpmsum_2way_safe),
      ("xz/vpmsum-4way", crc64_xz_vpmsum_4way_safe),
      ("xz/vpmsum-8way", crc64_xz_vpmsum_8way_safe),
    ] {
      assert_kernel(name, kernel, super::super::portable::crc64_slice16_xz);
    }

    for (name, kernel) in [
      ("nvme/vpmsum", crc64_nvme_vpmsum_safe as fn(u64, &[u8]) -> u64),
      ("nvme/vpmsum-2way", crc64_nvme_vpmsum_2way_safe),
      ("nvme/vpmsum-4way", crc64_nvme_vpmsum_4way_safe),
      ("nvme/vpmsum-8way", crc64_nvme_vpmsum_8way_safe),
    ] {
