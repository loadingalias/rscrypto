...
/// `crate::platform::caps().has(aarch64::PMULL_READY)`.
#[target_feature(enable = "aes", enable = "neon")]
pub(crate) unsafe fn crc64_xz_pmull(crc: u64, data: &[u8]) -> u64 {
...
/// `crate::platform::caps().has(aarch64::PMULL_READY)`.
#[target_feature(enable = "aes", enable = "neon")]
pub(crate) unsafe fn crc64_xz_pmull_small(crc: u64, data: &[u8]) -> u64 {
  // SAFETY: this function requires PMULL+NEON via `target_feature`; the caller guarantees the CPU
...
/// `crate::platform::caps().has(aarch64::PMULL_READY)`.
#[target_feature(enable = "aes", enable = "neon")]
pub(crate) unsafe fn crc64_nvme_pmull(crc: u64, data: &[u8]) -> u64 {
  // SAFETY: this function requires PMULL+NEON via `target_feature`; the caller guarantees the CPU
  // supports it (via `PMULL_READY`), and `crc64_pmull` is called with valid references.
  unsafe {
    crc64_pmull(
...
/// `crate::platform::caps().has(aarch64::PMULL_READY)`.
#[target_feature(enable = "aes", enable = "neon")]
pub(crate) unsafe fn crc64_nvme_pmull_small(crc: u64, data: &[u8]) -> u64 {
  // SAFETY: this function requires PMULL+NEON via `target_feature`; the caller guarantees the CPU
  // supports it (via `PMULL_READY`), and `crc64_pmull_small` is called with valid references.
  unsafe {
    crc64_pmull_small(
...
/// `crate::platform::caps().has(aarch64::PMULL_EOR3_READY)`.
#[target_feature(enable = "aes", enable = "neon", enable = "sha3")]
pub(crate) unsafe fn crc64_xz_pmull_eor3(crc: u64, data: &[u8]) -> u64 {
...
/// `crate::platform::caps().has(aarch64::PMULL_EOR3_READY)`.
#[target_feature(enable = "aes", enable = "neon", enable = "sha3")]
pub(crate) unsafe fn crc64_nvme_pmull_eor3(crc: u64, data: &[u8]) -> u64 {
  // SAFETY: this function requires PMULL+NEON+SHA3 via `target_feature`; the caller guarantees
  // the CPU supports it (via `PMULL_EOR3_READY`), and `crc64_pmull_eor3` is called with valid
  // references.
  unsafe {
...
/// `crate::platform::caps().has(aarch64::PMULL_EOR3_READY)`.
#[target_feature(enable = "aes", enable = "neon", enable = "sha3")]
pub(crate) unsafe fn crc64_xz_pmull_eor3_2way(crc: u64, data: &[u8]) -> u64 {
  // SAFETY: this function requires PMULL+NEON+SHA3 via `target_feature`; the caller guarantees
...
/// `crate::platform::caps().has(aarch64::PMULL_EOR3_READY)`.
#[target_feature(enable = "aes", enable = "neon", enable = "sha3")]
pub(crate) unsafe fn crc64_xz_pmull_eor3_3way(crc: u64, data: &[u8]) -> u64 {
  // SAFETY: this function requires PMULL+NEON+SHA3 via `target_feature`; the caller guarantees
  // the CPU supports it (via `PMULL_EOR3_READY`), and the called kernel is passed valid references.
  unsafe {
...
/// `crate::platform::caps().has(aarch64::PMULL_EOR3_READY)`.
#[target_feature(enable = "aes", enable = "neon", enable = "sha3")]
pub(crate) unsafe fn crc64_nvme_pmull_eor3_2way(crc: u64, data: &[u8]) -> u64 {
  // SAFETY: this function requires PMULL+NEON+SHA3 via `target_feature`; the caller guarantees
  // the CPU supports it (via `PMULL_EOR3_READY`), and the called kernel is passed valid references.
  unsafe {
    crc64_pmull_eor3_2way(
...
/// `crate::platform::caps().has(aarch64::PMULL_EOR3_READY)`.
#[target_feature(enable = "aes", enable = "neon", enable = "sha3")]
pub(crate) unsafe fn crc64_nvme_pmull_eor3_3way(crc: u64, data: &[u8]) -> u64 {
  // SAFETY: this function requires PMULL+NEON+SHA3 via `target_feature`; the caller guarantees
  // the CPU supports it (via `PMULL_EOR3_READY`), and the called kernel is passed valid references.
  unsafe {
    crc64_pmull_eor3_3way(
...
/// `crate::platform::caps().has(aarch64::PMULL_READY)`.
#[target_feature(enable = "aes", enable = "neon")]
pub(crate) unsafe fn crc64_xz_pmull_2way(crc: u64, data: &[u8]) -> u64 {
  // SAFETY: this function requires PMULL+NEON via `target_feature`; the caller guarantees the CPU
...
/// `crate::platform::caps().has(aarch64::PMULL_READY)`.
#[target_feature(enable = "aes", enable = "neon")]
pub(crate) unsafe fn crc64_xz_pmull_3way(crc: u64, data: &[u8]) -> u64 {
  // SAFETY: this function requires PMULL+NEON via `target_feature`; the caller guarantees the CPU
  // supports it (via `PMULL_READY`), and the called kernel is passed valid references.
  unsafe {
    crc64_pmull_3way(
...
/// `crate::platform::caps().has(aarch64::PMULL_READY)`.
#[target_feature(enable = "aes", enable = "neon")]
pub(crate) unsafe fn crc64_nvme_pmull_2way(crc: u64, data: &[u8]) -> u64 {
  // SAFETY: this function requires PMULL+NEON via `target_feature`; the caller guarantees the CPU
  // supports it (via `PMULL_READY`), and the called kernel is passed valid references.
  unsafe {
    crc64_pmull_2way(
...
/// `crate::platform::caps().has(aarch64::PMULL_READY)`.
#[target_feature(enable = "aes", enable = "neon")]
pub(crate) unsafe fn crc64_nvme_pmull_3way(crc: u64, data: &[u8]) -> u64 {
  // SAFETY: this function requires PMULL+NEON via `target_feature`; the caller guarantees the CPU
  // supports it (via `PMULL_READY`), and the called kernel is passed valid references.
  unsafe {
    crc64_pmull_3way(
