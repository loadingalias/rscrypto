...
fn load_block_bitrev(block: &Block) -> [Simd; 8] {
  ...
  for (lane, &[low, high]) in out.iter_mut().zip(block.as_chunks::<2>().0) {
    *lane = Simd::new(
      bitrev_bytes_u64(u64::from_le(high)),
      bitrev_bytes_u64(u64::from_le(low)),
    );
  }
  out
}
...
fn load_block_split_bitrev(block: &Block) -> ([u64; 8], [u64; 8]) {
  ...
  for ((hi_lane, lo_lane), &[low, high]) in hi.iter_mut().zip(&mut lo).zip(block.as_chunks::<2>().0) {
    *hi_lane = bitrev_bytes_u64(u64::from_le(high));
    *lo_lane = bitrev_bytes_u64(u64::from_le(low));
  }
...
/// Requires the RISC-V extensions enabled by the target-feature attribute.
#[target_feature(enable = "zbc")]
fn crc24_width32_zbc(mut state: u32, data: &[u8], keys: &[u64; 23]) -> u32 {
...
/// Requires the RISC-V extensions enabled by the target-feature attribute.
#[target_feature(enable = "zbc")]
fn crc24_width32_zbc_2way(mut state: u32, data: &[u8], keys: &[u64; 23]) -> u32 {
  // SAFETY: Every bit pattern is valid for Block; align_to returns
  // non-overlapping subslices of the original allocation.
  unsafe {
    let (left, middle, right) = data.align_to::<Block>();
...
/// Requires the RISC-V extensions enabled by the target-feature attribute.
#[target_feature(enable = "zbc")]
fn crc24_width32_zbc_4way(mut state: u32, data: &[u8], keys: &[u64; 23]) -> u32 {
  // SAFETY: Every bit pattern is valid for Block; align_to returns
  // non-overlapping subslices of the original allocation.
  unsafe {
    let (left, middle, right) = data.align_to::<Block>();
...
fn update_simd_zvbc_2way(state: u32, blocks: &[Block], fold_256b: (u64, u64), keys: &[u64; 23]) -> u32 {
  ...
  while i < even {
    ...
    let (b1_hi, b1_lo) = load_block_split_bitrev(&blocks[i.strict_add(1)]);
...
fn update_simd_zvbc_4way(
  ...
  while i < aligned {
    let (b0_hi, b0_lo) = load_block_split_bitrev(&blocks[i]);
    let (b1_hi, b1_lo) = load_block_split_bitrev(&blocks[i.strict_add(1)]);
    let (b2_hi, b2_lo) = load_block_split_bitrev(&blocks[i.strict_add(2)]);
    let (b3_hi, b3_lo) = load_block_split_bitrev(&blocks[i.strict_add(3)]);
...
/// Requires the RISC-V extensions enabled by the target-feature attribute.
#[target_feature(enable = "v", enable = "zvbc")]
fn crc24_width32_zvbc(mut state: u32, data: &[u8], keys: &[u64; 23]) -> u32 {
  // SAFETY: Every bit pattern is valid for Block; align_to returns
  // non-overlapping subslices of the original allocation.
  unsafe {
    let (left, middle, right) = data.align_to::<Block>();
...
/// Requires the RISC-V extensions enabled by the target-feature attribute.
#[target_feature(enable = "v", enable = "zvbc")]
fn crc24_width32_zvbc_2way(mut state: u32, data: &[u8], keys: &[u64; 23]) -> u32 {
  // SAFETY: Every bit pattern is valid for Block; align_to returns
  // non-overlapping subslices of the original allocation.
  unsafe {
    let (left, middle, right) = data.align_to::<Block>();
...
/// Requires the RISC-V extensions enabled by the target-feature attribute.
#[target_feature(enable = "v", enable = "zvbc")]
fn crc24_width32_zvbc_4way(mut state: u32, data: &[u8], keys: &[u64; 23]) -> u32 {
  // SAFETY: Every bit pattern is valid for Block; align_to returns
  // non-overlapping subslices of the original allocation.
  unsafe {
    let (left, middle, right) = data.align_to::<Block>();
...
#[inline]
pub(super) fn crc24_openpgp_zbc_safe(crc: u32, data: &[u8]) -> u32 {
...
#[inline]
pub(super) fn crc24_openpgp_zbc_2way_safe(crc: u32, data: &[u8]) -> u32 {
  let mut state = to_reflected_state(crc);
  // SAFETY: Dispatcher verifies Zbc before selecting this kernel.
...
#[inline]
pub(super) fn crc24_openpgp_zbc_4way_safe(crc: u32, data: &[u8]) -> u32 {
  let mut state = to_reflected_state(crc);
  // SAFETY: Dispatcher verifies Zbc before selecting this kernel.
...
#[inline]
pub(super) fn crc24_openpgp_zvbc_safe(crc: u32, data: &[u8]) -> u32 {
  let mut state = to_reflected_state(crc);
...
#[inline]
pub(super) fn crc24_openpgp_zvbc_2way_safe(crc: u32, data: &[u8]) -> u32 {
  let mut state = to_reflected_state(crc);
  // SAFETY: Dispatcher verifies Zvbc before selecting this kernel.
...
#[inline]
pub(super) fn crc24_openpgp_zvbc_4way_safe(crc: u32, data: &[u8]) -> u32 {
  let mut state = to_reflected_state(crc);
  // SAFETY: Dispatcher verifies Zvbc before selecting this kernel.
  ...
  fn assert_kernel(name: &str, kernel: fn(u32, &[u8]) -> u32) {
    let input: Vec<u8> = (0usize..4111)
      .map(|i| {
        ...
        low.wrapping_mul(37).wrapping_add(high)
      })
      .collect();
    for &state in STATES {
