//! s390x hardware-accelerated CRC-24/OPENPGP kernels (VGFM-style).
//!
...
impl Simd {
  ...
  /// Requires the s390x vector facility.
  #[target_feature(enable = "vector")]
  fn fold_16_reflected(self, coeff: Self, data_to_xor: Self) -> Self {
  ...
  /// Requires the s390x vector facility.
  #[target_feature(enable = "vector")]
  fn fold_width32_reflected(self, high: u64, low: u64) -> Self {
  ...
  /// Requires the s390x vector facility.
  #[target_feature(enable = "vector")]
  fn barrett_width32_reflected(self, poly: u64, mu: u64) -> u32 {
...
/// Requires the s390x vector facility.
#[target_feature(enable = "vector")]
fn finalize_lanes_width32_reflected(x: [Simd; 8], keys: &[u64; 23]) -> u32 {
  let mut res = x[7];
...
/// Requires the s390x vector facility.
#[target_feature(enable = "vector")]
fn update_simd_width32_reflected_bitrev(state: u32, first: &Block, rest: &[Block], keys: &[u64; 23]) -> u32 {
...
/// Requires the s390x vector facility.
#[target_feature(enable = "vector")]
fn fold_block_128_reflected_bitrev(x: &mut [Simd; 8], block: &Block, coeff: Simd) {
  let chunk = load_block_bitrev(block);
...
/// Requires the s390x vector facility.
#[target_feature(enable = "vector")]
fn update_simd_width32_reflected_bitrev_2way(
  state: u32,
  blocks: &[Block],
  fold_256b: (u64, u64),
  keys: &[u64; 23],
...
/// Requires the s390x vector facility.
#[target_feature(enable = "vector")]
fn update_simd_width32_reflected_bitrev_4way(
  state: u32,
  blocks: &[Block],
  fold_512b: (u64, u64),
  combine: &[(u64, u64); 3],
...
/// Requires the s390x vector facility.
#[target_feature(enable = "vector")]
fn crc24_width32_vgfm_bitrev(mut state: u32, data: &[u8], keys: &[u64; 23]) -> u32 {
...
/// Requires the s390x vector facility.
#[target_feature(enable = "vector")]
fn crc24_width32_vgfm_bitrev_2way(mut state: u32, data: &[u8], keys: &[u64; 23]) -> u32 {
  // SAFETY: Every bit pattern is valid for Block; align_to returns
  // non-overlapping subslices of the original allocation.
  unsafe {
    let (left, middle, right) = data.align_to::<Block>();
...
/// Requires the s390x vector facility.
#[target_feature(enable = "vector")]
fn crc24_width32_vgfm_bitrev_4way(mut state: u32, data: &[u8], keys: &[u64; 23]) -> u32 {
  // SAFETY: Every bit pattern is valid for Block; align_to returns
  // non-overlapping subslices of the original allocation.
  unsafe {
    let (left, middle, right) = data.align_to::<Block>();
...
/// CRC-24/OPENPGP VGFM kernel.
#[inline]
pub(super) fn crc24_openpgp_vgfm_safe(crc: u32, data: &[u8]) -> u32 {
  let mut state = to_reflected_state(crc);
  // SAFETY: Dispatcher verifies VECTOR facility before selecting this kernel.
  state = unsafe { crc24_width32_vgfm_bitrev(state, data, &CRC24_OPENPGP_KEYS_REFLECTED) };
  from_reflected_state(state)
}

#[inline]
pub(super) fn crc24_openpgp_vgfm_2way_safe(crc: u32, data: &[u8]) -> u32 {
  let mut state = to_reflected_state(crc);
  // SAFETY: Dispatcher verifies VECTOR facility before selecting this kernel.
  state = unsafe { crc24_width32_vgfm_bitrev_2way(state, data, &CRC24_OPENPGP_KEYS_REFLECTED) };
  from_reflected_state(state)
}

#[inline]
pub(super) fn crc24_openpgp_vgfm_4way_safe(crc: u32, data: &[u8]) -> u32 {
  let mut state = to_reflected_state(crc);
  // SAFETY: Dispatcher verifies VECTOR facility before selecting this kernel.
  state = unsafe { crc24_width32_vgfm_bitrev_4way(state, data, &CRC24_OPENPGP_KEYS_REFLECTED) };
  from_reflected_state(state)
}
...
mod tests {
  ...
  fn vgfm_kernels_match_portable() {
    ...
    let input: Vec<u8> = (0..4111)
      .map(|i| (i as u8).wrapping_mul(47).wrapping_add((i >> 8) as u8))
      .collect();
    for (name, kernel) in [
      ("openpgp/vgfm", crc24_openpgp_vgfm_safe as fn(u32, &[u8]) -> u32),
      ("openpgp/vgfm-2way", crc24_openpgp_vgfm_2way_safe),
      ("openpgp/vgfm-4way", crc24_openpgp_vgfm_4way_safe),
    ] {
      for &state in STATES {
