...
fn load_block_bitrev(block: &Block) -> [Simd; 8] {
  ...
  let mut i = 0usize;
  while i < 8 {
    let low = bitrev_bytes_u64(u64::from_le(block[i * 2]));
    let high = bitrev_bytes_u64(u64::from_le(block[i * 2 + 1]));
...
#[inline]
pub fn crc24_openpgp_vgfm_safe(crc: u32, data: &[u8]) -> u32 {
...
#[inline]
pub fn crc24_openpgp_vgfm_2way_safe(crc: u32, data: &[u8]) -> u32 {
  let mut state = to_reflected_state(crc);
  // SAFETY: Dispatcher verifies VECTOR facility before selecting this kernel.
...
#[inline]
pub fn crc24_openpgp_vgfm_4way_safe(crc: u32, data: &[u8]) -> u32 {
  let mut state = to_reflected_state(crc);
  // SAFETY: Dispatcher verifies VECTOR facility before selecting this kernel.
