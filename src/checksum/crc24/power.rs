...
impl Simd {
  ...
  unsafe fn vpmsumd(a: i64x2, b: i64x2) -> i64x2 {
    // SAFETY: Inline asm requires altivec+vsx+power8-vector+power8-crypto, ensured by this function's
  ...
  unsafe fn mul64(a: u64, b: u64) -> Self {
    // SAFETY: All target-feature-gated calls require altivec+vsx+power8-vector+power8-crypto, ensured
    // by this function's #[target_feature] attribute.
    unsafe {
...
unsafe fn crc24_width32_vpmsum_bitrev(mut state: u32, data: &[u8], keys: &[u64; 23]) -> u32 {
  // SAFETY: All target-feature-gated calls require altivec+vsx+power8-vector+power8-crypto, ensured
  // by this function's #[target_feature] attribute. align_to is sound because Simd is
...
mod tests {
  ...
  fn vpmsum_kernels_match_portable() {
    ...
    for (name, kernel) in [
      ...
      ("openpgp/vpmsum-2way", crc24_openpgp_vpmsum_2way_safe),
      ("openpgp/vpmsum-4way", crc24_openpgp_vpmsum_4way_safe),
      ("openpgp/vpmsum-8way", crc24_openpgp_vpmsum_8way_safe),
    ] {
      for &state in STATES {
