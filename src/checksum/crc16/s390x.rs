...
/// Requires the s390x vector facility.
#[target_feature(enable = "vector")]
fn update_simd_width32_reflected(state: u32, first: &Block, rest: &[Block], keys: &[u64; 23]) -> u32 {
...
/// Requires the s390x vector facility.
#[target_feature(enable = "vector")]
fn fold_block_128_reflected(x: &mut [Simd; 8], block: &Block, coeff: Simd) {
  let chunk = load_block(block);
...
/// Requires the s390x vector facility.
#[target_feature(enable = "vector")]
fn update_simd_width32_reflected_2way(state: u32, blocks: &[Block], fold_256b: (u64, u64), keys: &[u64; 23]) -> u32 {
...
/// Requires the s390x vector facility.
#[target_feature(enable = "vector")]
fn crc16_width32_vgfm(mut state: u16, data: &[u8], keys: &[u64; 23], portable: fn(u16, &[u8]) -> u16) -> u16 {
...
#[inline]
pub(super) fn crc16_ccitt_vgfm_safe(crc: u16, data: &[u8]) -> u16 {
...
#[inline]
pub(super) fn crc16_ccitt_vgfm_2way_safe(crc: u16, data: &[u8]) -> u16 {
  // SAFETY: Dispatcher verifies VECTOR facility before selecting this kernel.
  unsafe {
...
#[inline]
pub(super) fn crc16_ccitt_vgfm_4way_safe(crc: u16, data: &[u8]) -> u16 {
  // SAFETY: Dispatcher verifies VECTOR facility before selecting this kernel.
  unsafe {
...
#[inline]
pub(super) fn crc16_ibm_vgfm_safe(crc: u16, data: &[u8]) -> u16 {
  // SAFETY: Dispatcher verifies VECTOR facility before selecting this kernel.
  unsafe { crc16_width32_vgfm(crc, data, &CRC16_IBM_KEYS_REFLECTED, super::portable::crc16_ibm_slice8) }
}

#[inline]
pub(super) fn crc16_ibm_vgfm_2way_safe(crc: u16, data: &[u8]) -> u16 {
  // SAFETY: Dispatcher verifies VECTOR facility before selecting this kernel.
  unsafe {
    crc16_width32_vgfm_2way(
      crc,
...
#[inline]
pub(super) fn crc16_ibm_vgfm_4way_safe(crc: u16, data: &[u8]) -> u16 {
  // SAFETY: Dispatcher verifies VECTOR facility before selecting this kernel.
  unsafe {
    crc16_width32_vgfm_4way(
      crc,
  ...
  fn assert_kernel(name: &str, kernel: fn(u16, &[u8]) -> u16, portable: fn(u16, &[u8]) -> u16) {
    let input: Vec<u8> = (0usize..4111)
      .map(|i| {
        ...
        low.wrapping_mul(43).wrapping_add(high)
      })
      .collect();
    for &state in STATES {
