...
// ---------------------------------------------------------------------------
// AVX-512 IFMA point operations
// ---------------------------------------------------------------------------
//
// Mirrors the AVX2 point operations but calls `mul_ifma` / `square_ifma`
// for ~50% fewer instructions in the inner field multiply loops.
// All shuffle/blend/add/sub/reduce operations are shared with AVX2.

#[cfg(target_arch = "x86_64")]
impl ExtendedPointAvx2 {
  /// IFMA variant of `to_cached`.
  ///
  /// # Safety
  ///
  ...
  #[allow(unsafe_op_in_unsafe_fn)]
  pub(crate) unsafe fn to_cached_ifma(self) -> CachedPointAvx2 {
    let ds = self.0.diff_sum();
    let prepared = self.0.blend(&ds, Lanes::AB);
    let constants = hamburg_constants();
    let scaled = prepared.mul_ifma(&constants);
    let negated = scaled.negate_lazy();
    CachedPointAvx2(scaled.blend(&negated, Lanes::D))
  }

  /// IFMA variant of `add_cached`.
  ///
  /// # Safety
  ///
  /// Caller must ensure AVX-512 IFMA + VL are available.
  ...
  #[target_feature(enable = "avx2,avx512ifma,avx512vl")]
  #[allow(unsafe_op_in_unsafe_fn)]
  pub(crate) unsafe fn add_cached_ifma(&self, other: &CachedPointAvx2) -> Self {
    let ds = self.0.diff_sum();
    let tmp = self.0.blend(&ds, Lanes::AB);
    let product = tmp.mul_ifma(&other.0);
    let swapped = product.shuffle(Shuffle::ABDC);
    let ehfg = swapped.diff_sum();
  ...
  #[target_feature(enable = "avx2,avx512ifma,avx512vl")]
  #[allow(unsafe_op_in_unsafe_fn)]
  pub(crate) unsafe fn double_ifma(&self) -> Self {
    ...
    let sq = prepared.square_and_negate_d_ifma();
...
// ---------------------------------------------------------------------------
// AVX-512 IFMA scalar multiplication
// ---------------------------------------------------------------------------

/// IFMA variant of `cached_from_affine`.
///
/// # Safety
///
/// Caller must ensure AVX-512 IFMA + VL are available.
...
#[target_feature(enable = "avx2,avx512ifma,avx512vl")]
#[allow(unsafe_op_in_unsafe_fn)]
unsafe fn cached_from_affine_ifma(cp: &CachedPoint, constants: &FieldElement2625x4) -> CachedPointAvx2 {
  let (y_plus_x, y_minus_x, t2d) = cp.components();
  let packed = FieldElement2625x4::new(y_minus_x, y_plus_x, &FieldElement::ONE, t2d);
  CachedPointAvx2(packed.mul_ifma(constants))
}

/// IFMA variant of `add_signed_cached_avx2`.
#[inline]
#[target_feature(enable = "avx2,avx512ifma,avx512vl")]
#[allow(unsafe_op_in_unsafe_fn)]
unsafe fn add_signed_cached_ifma(
  ...
  if digit > 0 {
    acc.add_cached_ifma(&cached)
  } else {
    acc.add_cached_ifma(&cached.neg())
  }
}

/// IFMA variant of `add_signed_runtime_cached_avx2`.
#[inline]
#[target_feature(enable = "avx2,avx512ifma,avx512vl")]
#[allow(unsafe_op_in_unsafe_fn)]
unsafe fn add_signed_runtime_cached_ifma(
  ...
  if digit > 0 {
    acc.add_cached_ifma(point)
  } else {
    acc.add_cached_ifma(&point.neg())
  }
}

/// IFMA variant of `cached_multiples_avx2`.
#[target_feature(enable = "avx2,avx512ifma,avx512vl")]
#[allow(unsafe_op_in_unsafe_fn)]
unsafe fn cached_multiples_ifma(point: &ExtendedPointAvx2) -> [CachedPointAvx2; 8] {
  let mut acc = *point;
  let point_cached = point.to_cached_ifma();
  let first = acc.to_cached_ifma();

  let mut out = [first; 8];
  for entry in out.iter_mut().skip(1) {
    acc = acc.add_cached_ifma(&point_cached);
    *entry = acc.to_cached_ifma();
  }
  out
}
...
pub(crate) unsafe fn scalar_mul_vartime_ifma(point: &ExtendedPoint, scalar_bytes: &[u8; 32]) -> ExtendedPoint {
  let digits = scalar::as_radix_16(scalar_bytes);
  let avx_point = ExtendedPointAvx2::from_extended(point);
  let table = cached_multiples_ifma(&avx_point);
  let mut acc = ExtendedPointAvx2::from_extended(&ExtendedPoint::identity());
  for digit in digits.iter().rev().copied() {
    acc = acc.double_ifma().double_ifma().double_ifma().double_ifma();
    if digit != 0 {
...
/// Fixed-base scalar multiplication using AVX-512 IFMA.
///
/// # Safety
///
/// Caller must ensure AVX-512 IFMA + VL are available.
...
/// Straus/Shamir interleaved double-scalar multiply using AVX-512 IFMA.
///
/// Scans both scalars in lockstep over 64 signed radix-16 digits
/// (high → low), sharing all 256 doublings via IFMA parallel field ops.
///
/// # Safety
///
/// Caller must ensure AVX-512 IFMA + VL are available.
...
pub(crate) unsafe fn straus_basepoint_vartime_ifma(s: &[u8; 32], h: &[u8; 32], a: &ExtendedPoint) -> ExtendedPoint {
  ...
  let avx_a = ExtendedPointAvx2::from_extended(a);
  let a_table = cached_multiples_ifma(&avx_a);
