...
impl FieldElement51x4 {
  ...
  /// Multiply with asymmetric input widths: self ≤ 54 bits, rhs ≤ 53 bits.
  ///
  /// Designed for HWCD'08 doubling where lane C (S8 = S1 + 2S3 − S2)
  /// reaches ~53.3 bits due to the `negate_lazy` bias, while the rhs
  /// operand stays within 53 bits.
  ///
  /// The self operand's 2-bit overflow (above 52 bits) is decomposed
  /// into bit 0 and bit 1. The IFMA schoolbook computes `f_lo × g_lo`,
  /// and corrections handle the overflow via dual `select_by_bit`:
  ///
  ///   `f_hi × g_lo = bit0(f_hi) × g_lo + bit1(f_hi) × (2 × g_lo)`
  ///
  /// The rhs overflow is 1-bit, handled identically to `mul_unreduced`.
  ///
  /// # Precondition
  ///
  /// - `self`: limbs ≤ 54 bits (2 excess bits above IFMA's 52-bit window)
  /// - `rhs`: limbs ≤ 53 bits (1 excess bit above IFMA's 52-bit window)
  ///
  /// # Overflow safety
  ///
  /// IFMA lo/hi accumulators reach ~2^54.3 (5 terms). 2-bit f_hi
  /// corrections add ≤ 5 × 2^53 ≈ 2^55.3 per hi_k. 1-bit g_hi
  /// corrections add ≤ 5 × 2^52 ≈ 2^54.3 per hi_k. After recombine:
  /// z_k ≤ 2^57. After ×19 fold: ≤ 2^61.3. Well within u64.
  ///
  /// # Safety
  ///
  /// Caller must ensure AVX-512 IFMA + VL are available.
  #[target_feature(enable = "avx2,avx512ifma,avx512vl")]
  #[allow(unsafe_op_in_unsafe_fn)]
  pub(crate) unsafe fn mul_wide_unreduced(&self, rhs: &Self) -> Self {
    let zero = _mm256_setzero_si256();
    let mask52 = _mm256_set1_epi64x(MASK52);
    let one = _mm256_set1_epi64x(1);

    // Pre-mask self to 52 bits; extract 2-bit overflow.
    let f_lo: [__m256i; 5] = [
      _mm256_and_si256(self.0[0], mask52),
      _mm256_and_si256(self.0[1], mask52),
      _mm256_and_si256(self.0[2], mask52),
    ...
    // Decompose f_hi into bit 0 and bit 1 for 2-bit correction.
    // f_hi ≤ 2 in practice (max S8 ≈ 1.25 × 2^53, >> 52 = 2).
    let f_hi_b0: [__m256i; 5] = [
      _mm256_and_si256(f_hi[0], one),
      _mm256_and_si256(f_hi[1], one),
      _mm256_and_si256(f_hi[2], one),
      _mm256_and_si256(f_hi[3], one),
      _mm256_and_si256(f_hi[4], one),
    ];
    let f_hi_b1: [__m256i; 5] = [
      _mm256_srli_epi64::<1>(f_hi[0]),
      _mm256_srli_epi64::<1>(f_hi[1]),
      _mm256_srli_epi64::<1>(f_hi[2]),
      _mm256_srli_epi64::<1>(f_hi[3]),
      _mm256_srli_epi64::<1>(f_hi[4]),
    ];

    // Pre-mask rhs to 52 bits; extract 1-bit overflow.
    let g_lo: [__m256i; 5] = [
      _mm256_and_si256(rhs.0[0], mask52),
      _mm256_and_si256(rhs.0[1], mask52),
      _mm256_and_si256(rhs.0[2], mask52),
    ...
    // Pre-double g_lo for bit-1 corrections: bit1(f_hi) × (2 × g_lo).
    let g_lo_2: [__m256i; 5] = [
      _mm256_add_epi64(g_lo[0], g_lo[0]),
      _mm256_add_epi64(g_lo[1], g_lo[1]),
      _mm256_add_epi64(g_lo[2], g_lo[2]),
      _mm256_add_epi64(g_lo[3], g_lo[3]),
      _mm256_add_epi64(g_lo[4], g_lo[4]),
    ];

    // -----------------------------------------------------------------------
    // Phase 1: 5×5 schoolbook on 52-bit inputs (50 IFMA ops)
    // Identical to mul_unreduced — interleaved lo/hi scheduling.
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // Phase 1b: 2-bit f_hi × g_lo corrections
    //
    // f_hi[i] × g_lo[j] = bit0 × g_lo + bit1 × (2 × g_lo)
    // Both go into hi accumulators (weight 2 × 2^51 from overflow position).
    // -----------------------------------------------------------------------

    // k=0: (0,0)
    let hi0 = _mm256_add_epi64(hi0, select_by_bit(f_hi_b0[0], g_lo[0]));
    let hi0 = _mm256_add_epi64(hi0, select_by_bit(f_hi_b1[0], g_lo_2[0]));

    // k=1: (0,1), (1,0)
    let mut hi1 = _mm256_add_epi64(hi1, select_by_bit(f_hi_b0[0], g_lo[1]));
    hi1 = _mm256_add_epi64(hi1, select_by_bit(f_hi_b1[0], g_lo_2[1]));
    hi1 = _mm256_add_epi64(hi1, select_by_bit(f_hi_b0[1], g_lo[0]));
    hi1 = _mm256_add_epi64(hi1, select_by_bit(f_hi_b1[1], g_lo_2[0]));

    // k=2: (0,2), (1,1), (2,0)
    let mut hi2 = _mm256_add_epi64(hi2, select_by_bit(f_hi_b0[0], g_lo[2]));
    hi2 = _mm256_add_epi64(hi2, select_by_bit(f_hi_b1[0], g_lo_2[2]));
    hi2 = _mm256_add_epi64(hi2, select_by_bit(f_hi_b0[1], g_lo[1]));
    hi2 = _mm256_add_epi64(hi2, select_by_bit(f_hi_b1[1], g_lo_2[1]));
    hi2 = _mm256_add_epi64(hi2, select_by_bit(f_hi_b0[2], g_lo[0]));
    hi2 = _mm256_add_epi64(hi2, select_by_bit(f_hi_b1[2], g_lo_2[0]));

    // k=3: (0,3), (1,2), (2,1), (3,0)
    let mut hi3 = _mm256_add_epi64(hi3, select_by_bit(f_hi_b0[0], g_lo[3]));
    hi3 = _mm256_add_epi64(hi3, select_by_bit(f_hi_b1[0], g_lo_2[3]));
    hi3 = _mm256_add_epi64(hi3, select_by_bit(f_hi_b0[1], g_lo[2]));
    hi3 = _mm256_add_epi64(hi3, select_by_bit(f_hi_b1[1], g_lo_2[2]));
    hi3 = _mm256_add_epi64(hi3, select_by_bit(f_hi_b0[2], g_lo[1]));
    hi3 = _mm256_add_epi64(hi3, select_by_bit(f_hi_b1[2], g_lo_2[1]));
    hi3 = _mm256_add_epi64(hi3, select_by_bit(f_hi_b0[3], g_lo[0]));
    hi3 = _mm256_add_epi64(hi3, select_by_bit(f_hi_b1[3], g_lo_2[0]));

    // k=4: (0,4), (1,3), (2,2), (3,1), (4,0)
    let mut hi4 = _mm256_add_epi64(hi4, select_by_bit(f_hi_b0[0], g_lo[4]));
    hi4 = _mm256_add_epi64(hi4, select_by_bit(f_hi_b1[0], g_lo_2[4]));
    hi4 = _mm256_add_epi64(hi4, select_by_bit(f_hi_b0[1], g_lo[3]));
    hi4 = _mm256_add_epi64(hi4, select_by_bit(f_hi_b1[1], g_lo_2[3]));
    hi4 = _mm256_add_epi64(hi4, select_by_bit(f_hi_b0[2], g_lo[2]));
    hi4 = _mm256_add_epi64(hi4, select_by_bit(f_hi_b1[2], g_lo_2[2]));
    hi4 = _mm256_add_epi64(hi4, select_by_bit(f_hi_b0[3], g_lo[1]));
    hi4 = _mm256_add_epi64(hi4, select_by_bit(f_hi_b1[3], g_lo_2[1]));
    hi4 = _mm256_add_epi64(hi4, select_by_bit(f_hi_b0[4], g_lo[0]));
    hi4 = _mm256_add_epi64(hi4, select_by_bit(f_hi_b1[4], g_lo_2[0]));

    // k=5: (1,4), (2,3), (3,2), (4,1)
    let mut hi5 = _mm256_add_epi64(hi5, select_by_bit(f_hi_b0[1], g_lo[4]));
    hi5 = _mm256_add_epi64(hi5, select_by_bit(f_hi_b1[1], g_lo_2[4]));
    hi5 = _mm256_add_epi64(hi5, select_by_bit(f_hi_b0[2], g_lo[3]));
    hi5 = _mm256_add_epi64(hi5, select_by_bit(f_hi_b1[2], g_lo_2[3]));
    hi5 = _mm256_add_epi64(hi5, select_by_bit(f_hi_b0[3], g_lo[2]));
    hi5 = _mm256_add_epi64(hi5, select_by_bit(f_hi_b1[3], g_lo_2[2]));
    hi5 = _mm256_add_epi64(hi5, select_by_bit(f_hi_b0[4], g_lo[1]));
    hi5 = _mm256_add_epi64(hi5, select_by_bit(f_hi_b1[4], g_lo_2[1]));

    // k=6: (2,4), (3,3), (4,2)
    let mut hi6 = _mm256_add_epi64(hi6, select_by_bit(f_hi_b0[2], g_lo[4]));
    hi6 = _mm256_add_epi64(hi6, select_by_bit(f_hi_b1[2], g_lo_2[4]));
    hi6 = _mm256_add_epi64(hi6, select_by_bit(f_hi_b0[3], g_lo[3]));
    hi6 = _mm256_add_epi64(hi6, select_by_bit(f_hi_b1[3], g_lo_2[3]));
    hi6 = _mm256_add_epi64(hi6, select_by_bit(f_hi_b0[4], g_lo[2]));
    hi6 = _mm256_add_epi64(hi6, select_by_bit(f_hi_b1[4], g_lo_2[2]));

    // k=7: (3,4), (4,3)
    let mut hi7 = _mm256_add_epi64(hi7, select_by_bit(f_hi_b0[3], g_lo[4]));
    hi7 = _mm256_add_epi64(hi7, select_by_bit(f_hi_b1[3], g_lo_2[4]));
    hi7 = _mm256_add_epi64(hi7, select_by_bit(f_hi_b0[4], g_lo[3]));
    hi7 = _mm256_add_epi64(hi7, select_by_bit(f_hi_b1[4], g_lo_2[3]));

    // k=8: (4,4)
    let hi8 = _mm256_add_epi64(hi8, select_by_bit(f_hi_b0[4], g_lo[4]));
    let hi8 = _mm256_add_epi64(hi8, select_by_bit(f_hi_b1[4], g_lo_2[4]));

    // -----------------------------------------------------------------------
    // Phase 1c: 1-bit g_hi × f_lo corrections (same as mul_unreduced)
    // -----------------------------------------------------------------------

    let hi0 = _mm256_add_epi64(hi0, select_by_bit(g_hi[0], f_lo[0]));

    let mut hi1 = _mm256_add_epi64(hi1, select_by_bit(g_hi[1], f_lo[0]));
    hi1 = _mm256_add_epi64(hi1, select_by_bit(g_hi[0], f_lo[1]));

    let mut hi2 = _mm256_add_epi64(hi2, select_by_bit(g_hi[2], f_lo[0]));
    hi2 = _mm256_add_epi64(hi2, select_by_bit(g_hi[1], f_lo[1]));
    hi2 = _mm256_add_epi64(hi2, select_by_bit(g_hi[0], f_lo[2]));

    let mut hi3 = _mm256_add_epi64(hi3, select_by_bit(g_hi[3], f_lo[0]));
    hi3 = _mm256_add_epi64(hi3, select_by_bit(g_hi[2], f_lo[1]));
    hi3 = _mm256_add_epi64(hi3, select_by_bit(g_hi[1], f_lo[2]));
    hi3 = _mm256_add_epi64(hi3, select_by_bit(g_hi[0], f_lo[3]));

    let mut hi4 = _mm256_add_epi64(hi4, select_by_bit(g_hi[4], f_lo[0]));
    hi4 = _mm256_add_epi64(hi4, select_by_bit(g_hi[3], f_lo[1]));
    hi4 = _mm256_add_epi64(hi4, select_by_bit(g_hi[2], f_lo[2]));
    hi4 = _mm256_add_epi64(hi4, select_by_bit(g_hi[1], f_lo[3]));
    hi4 = _mm256_add_epi64(hi4, select_by_bit(g_hi[0], f_lo[4]));

    let mut hi5 = _mm256_add_epi64(hi5, select_by_bit(g_hi[4], f_lo[1]));
    hi5 = _mm256_add_epi64(hi5, select_by_bit(g_hi[3], f_lo[2]));
    hi5 = _mm256_add_epi64(hi5, select_by_bit(g_hi[2], f_lo[3]));
    hi5 = _mm256_add_epi64(hi5, select_by_bit(g_hi[1], f_lo[4]));

    let mut hi6 = _mm256_add_epi64(hi6, select_by_bit(g_hi[4], f_lo[2]));
    hi6 = _mm256_add_epi64(hi6, select_by_bit(g_hi[3], f_lo[3]));
    hi6 = _mm256_add_epi64(hi6, select_by_bit(g_hi[2], f_lo[4]));

    let mut hi7 = _mm256_add_epi64(hi7, select_by_bit(g_hi[4], f_lo[3]));
    hi7 = _mm256_add_epi64(hi7, select_by_bit(g_hi[3], f_lo[4]));
    ...
    // Phase 2b: f_hi × g_hi corrections
    //
    // f_hi can be 0-2, g_hi is 0-1. Use select_by_bit(g_hi, f_hi) which
    // gives 0 when g_hi=0, f_hi when g_hi=1 — correct for all f_hi values.
    // (AND would silently truncate f_hi=2 to 0.)
    // -----------------------------------------------------------------------

    // k=0 → z2
    let hh_0 = select_by_bit(g_hi[0], f_hi[0]);
    let z2 = _mm256_add_epi64(z2, _mm256_slli_epi64::<2>(hh_0));

    // k=1 → z3
    let hh_1 = _mm256_add_epi64(select_by_bit(g_hi[1], f_hi[0]), select_by_bit(g_hi[0], f_hi[1]));
    let z3 = _mm256_add_epi64(z3, _mm256_slli_epi64::<2>(hh_1));

    // k=2 → z4
    let hh_2 = _mm256_add_epi64(
      _mm256_add_epi64(select_by_bit(g_hi[2], f_hi[0]), select_by_bit(g_hi[1], f_hi[1])),
      select_by_bit(g_hi[0], f_hi[2]),
    );
    let z4 = _mm256_add_epi64(z4, _mm256_slli_epi64::<2>(hh_2));

    // k=3 → z5
    let hh_3 = _mm256_add_epi64(
      _mm256_add_epi64(select_by_bit(g_hi[3], f_hi[0]), select_by_bit(g_hi[2], f_hi[1])),
      _mm256_add_epi64(select_by_bit(g_hi[1], f_hi[2]), select_by_bit(g_hi[0], f_hi[3])),
    );
    let z5 = _mm256_add_epi64(z5, _mm256_slli_epi64::<2>(hh_3));

    // k=4 → z6
    let hh_4 = _mm256_add_epi64(
      _mm256_add_epi64(select_by_bit(g_hi[4], f_hi[0]), select_by_bit(g_hi[3], f_hi[1])),
      _mm256_add_epi64(
        _mm256_add_epi64(select_by_bit(g_hi[2], f_hi[2]), select_by_bit(g_hi[1], f_hi[3])),
        select_by_bit(g_hi[0], f_hi[4]),
      ),
    );
    let z6 = _mm256_add_epi64(z6, _mm256_slli_epi64::<2>(hh_4));
    ...
    // k=5 → z7
    let hh_5 = _mm256_add_epi64(
      _mm256_add_epi64(select_by_bit(g_hi[4], f_hi[1]), select_by_bit(g_hi[3], f_hi[2])),
      _mm256_add_epi64(select_by_bit(g_hi[2], f_hi[3]), select_by_bit(g_hi[1], f_hi[4])),
    );
    let z7 = _mm256_add_epi64(z7, _mm256_slli_epi64::<2>(hh_5));

    // k=6 → z8
    let hh_6 = _mm256_add_epi64(
      _mm256_add_epi64(select_by_bit(g_hi[4], f_hi[2]), select_by_bit(g_hi[3], f_hi[3])),
      select_by_bit(g_hi[2], f_hi[4]),
    );
    let z8 = _mm256_add_epi64(z8, _mm256_slli_epi64::<2>(hh_6));

    // k=7 → z9
    let hh_7 = _mm256_add_epi64(select_by_bit(g_hi[4], f_hi[3]), select_by_bit(g_hi[3], f_hi[4]));
    let z9 = _mm256_add_epi64(z9, _mm256_slli_epi64::<2>(hh_7));

    // k=8 → fold via ×19 into z5 (same as mul_unreduced)
    let hh_8 = select_by_bit(g_hi[4], f_hi[4]);
    let hh_8_x4 = _mm256_slli_epi64::<2>(hh_8);
    let z5 = _mm256_add_epi64(z5, mul19(hh_8_x4));
  ...
  ///
  /// This tighter bound is critical: with all lanes ≤51 bits, the
  /// doubling intermediates stay within `mul_wide_unreduced`'s 54-bit
  /// self / 53-bit rhs limits, eliminating the serial `.reduce()` that
  /// previously sat on the critical path in `double()`.
  ///
  /// # Precondition
  ///
  /// All limbs must be ≤ 52 bits.
...
mod tests {
  ...
  #[test]
  fn mul_wide_unreduced_matches_reduce_then_mul() {
    if !is_x86_feature_detected!("avx512ifma") {
      return;
    }
    // 54-bit self (simulating S8 = S1 + 2S3 + negate_lazy(S2) in double()).
    let lhs_fes = [
      FieldElement::from_limbs([
        (1u64 << 53) + (1u64 << 51),
        (1u64 << 53) + (1u64 << 51) - 5,
      ...
      // Normal 51-bit lane (S5 in double — no overflow)
      FieldElement::from_limbs([
        (1u64 << 51) - 1,
        (1u64 << 51) - 42,
        (1u64 << 50),
      ...
      // 54-bit lane (S8 again in the CACA shuffle)
      FieldElement::from_limbs([
        (1u64 << 53) + (1u64 << 51),
        (1u64 << 53) + (1u64 << 51) - 5,
        (1u64 << 52) + 12345,
    ...
    // 53-bit rhs (simulating S6/S9 in double()).
    let rhs_fes = test_53bit_field_elements();

    // SAFETY: AVX-512 IFMA checked above.
    unsafe {
      let lhs = FieldElement51x4::new(&lhs_fes[0], &lhs_fes[1], &lhs_fes[2], &lhs_fes[3]);
      let rhs = FieldElement51x4::new(&rhs_fes[0], &rhs_fes[1], &rhs_fes[2], &rhs_fes[3]);

      let via_wide = lhs.mul_wide_unreduced(&rhs);
      let [wa, wb, wc, wd] = via_wide.split();

      // Reference: reduce both operands to ≤51 bits, then standard mul.
      let lhs_reduced = lhs.reduce();
      let rhs_reduced = rhs.reduce();
      let via_reduced = lhs_reduced.mul(&rhs_reduced);
      let [ra, rb, rc, rd] = via_reduced.split();

      assert_eq!(wa.normalize(), ra.normalize(), "lane A mul_wide vs reduce+mul");
      assert_eq!(wb.normalize(), rb.normalize(), "lane B mul_wide vs reduce+mul");
      assert_eq!(wc.normalize(), rc.normalize(), "lane C mul_wide vs reduce+mul");
      assert_eq!(wd.normalize(), rd.normalize(), "lane D mul_wide vs reduce+mul");
    }
  }

  #[test]
  fn mul_wide_unreduced_51bit_matches_mul_unreduced() {
    if !is_x86_feature_detected!("avx512ifma") {
      return;
    }
    // With 51-bit inputs, mul_wide_unreduced should match mul_unreduced
    // (all overflow bits are zero).
    let [a, b, c, d] = test_field_elements();
    let [e, f, g, h] = [
      FieldElement::from_limbs([100_000_000, 200_000_000, 300_000_000, 400_000_000, 500_000_000]),
      FieldElement::from_limbs([600_000_000, 700_000_000, 800_000_000, 900_000_000, 1_000_000_000]),
    ...
    unsafe {
      ...
      assert_eq!(
        wa.normalize(),
        ua.normalize(),
        "lane A mul_wide vs mul_unreduced (51-bit)"
      );
      assert_eq!(
        wb.normalize(),
        ub.normalize(),
        "lane B mul_wide vs mul_unreduced (51-bit)"
      );
      assert_eq!(
        wc.normalize(),
        uc.normalize(),
        "lane C mul_wide vs mul_unreduced (51-bit)"
      );
      assert_eq!(
        wd.normalize(),
        ud.normalize(),
        "lane D mul_wide vs mul_unreduced (51-bit)"
      );
    }
  }
