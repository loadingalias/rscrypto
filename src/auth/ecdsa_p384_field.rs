//! Fiat-Crypto P-384 field arithmetic used by portable ECDSA paths.
//!
//! This file contains a narrow, adapted extraction of generated Fiat-Crypto code
...
#[inline(always)]
fn fiat_p384_addcarryx_u64(carry: u8, lhs: u64, rhs: u64) -> (u64, u8) {
  let (sum, overflow0) = lhs.overflowing_add(rhs);
  let (sum, overflow1) = sum.overflowing_add(u64::from(carry));
  (sum, u8::from(overflow0 | overflow1))
}

#[inline(always)]
fn fiat_p384_subborrowx_u64(borrow: u8, lhs: u64, rhs: u64) -> (u64, u8) {
  let (difference, underflow0) = lhs.overflowing_sub(rhs);
  let (difference, underflow1) = difference.overflowing_sub(u64::from(borrow));
  (difference, u8::from(underflow0 | underflow1))
}
...
#[inline(always)]
fn fiat_p384_cmovznz_u64(choice: u8, zero: u64, nonzero: u64) -> u64 {
  let mask = u64::from(choice).wrapping_neg();
  (mask & nonzero) | (!mask & zero)
}

fn fiat_p384_mul(arg1: &[u64; 6], arg2: &[u64; 6]) -> [u64; 6] {
  let mut out1 = [0; 6];
  let x1: u64 = arg1[1];
  let x2: u64 = arg1[2];
  let x3: u64 = arg1[3];
  ...
  let x29: u64 = u64::from(x28).wrapping_add(x8);
  let (x30, _x31) = fiat_p384_mulx_u64(x17, 0x100000001);
  ...
  let x54: u64 = u64::from(x53).wrapping_add(x33);
  let (_x55, x56) = fiat_p384_addcarryx_u64(0x0, x17, x42);
  ...
  let x91: u64 = u64::from(x90).wrapping_add(x70);
  ...
  let (x104, x105) = fiat_p384_addcarryx_u64(x103, u64::from(x68), x91);
  let (x106, _x107) = fiat_p384_mulx_u64(x92, 0x100000001);
  ...
  let x130: u64 = u64::from(x129).wrapping_add(x109);
  let (_x131, x132) = fiat_p384_addcarryx_u64(0x0, x92, x118);
  ...
  let x145: u64 = u64::from(x144).wrapping_add(u64::from(x105));
  ...
  let x168: u64 = u64::from(x167).wrapping_add(x147);
  ...
  let (x183, _x184) = fiat_p384_mulx_u64(x169, 0x100000001);
  ...
  let x207: u64 = u64::from(x206).wrapping_add(x186);
  let (_x208, x209) = fiat_p384_addcarryx_u64(0x0, x169, x195);
  ...
  let x222: u64 = u64::from(x221).wrapping_add(u64::from(x182));
  ...
  let x245: u64 = u64::from(x244).wrapping_add(x224);
  ...
  let (x260, _x261) = fiat_p384_mulx_u64(x246, 0x100000001);
  ...
  let x284: u64 = u64::from(x283).wrapping_add(x263);
  let (_x285, x286) = fiat_p384_addcarryx_u64(0x0, x246, x272);
  ...
  let x299: u64 = u64::from(x298).wrapping_add(u64::from(x259));
  ...
  let x322: u64 = u64::from(x321).wrapping_add(x301);
  ...
  let (x337, _x338) = fiat_p384_mulx_u64(x323, 0x100000001);
  ...
  let x361: u64 = u64::from(x360).wrapping_add(x340);
  let (_x362, x363) = fiat_p384_addcarryx_u64(0x0, x323, x349);
  ...
  let x376: u64 = u64::from(x375).wrapping_add(u64::from(x336));
  ...
  let x399: u64 = u64::from(x398).wrapping_add(x378);
  ...
  let (x414, _x415) = fiat_p384_mulx_u64(x400, 0x100000001);
  ...
  let x438: u64 = u64::from(x437).wrapping_add(x417);
  let (_x439, x440) = fiat_p384_addcarryx_u64(0x0, x400, x426);
  ...
  let x453: u64 = u64::from(x452).wrapping_add(u64::from(x413));
  ...
  let (_x466, x467) = fiat_p384_subborrowx_u64(x465, x453, 0x0_u64);
...
fn fiat_p384_square(arg1: &[u64; 6]) -> [u64; 6] {
  let mut out1 = [0; 6];
  let x1: u64 = arg1[1];
  let x2: u64 = arg1[2];
  let x3: u64 = arg1[3];
