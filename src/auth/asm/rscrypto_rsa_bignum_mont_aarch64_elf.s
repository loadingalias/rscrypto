...
//
// Adapted for rscrypto from BoringSSL generated armv8-mont-apple.S and retargeted for ELF.
...
.globl	rscrypto_rsa_bn_mul_mont_words_aarch64_elf
.hidden	rscrypto_rsa_bn_mul_mont_words_aarch64_elf

.align	5
rscrypto_rsa_bn_mul_mont_words_aarch64_elf:
	tst	x5,#7
	b.eq	Lrscrypto_rsa_bn_sqr8x_mont
	tst	x5,#3
	b.eq	Lrscrypto_rsa_bn_mul4x_mont
Lmul_mont:
	stp	x29,x30,[sp,#-64]!
	add	x29,sp,#0
	stp	x19,x20,[sp,#16]
...
Lsqr8x_mont:
	...
	// pages. If [x2] were the next access, we would skip a guard page.
	//
	// Fortunately, the first access is [x2,#8*8], at .Lsqr8x_zero_start.
