...
// The public symbol is renamed to the rscrypto namespace and embedded with Rust global_asm!.
        ...
        .globl _rscrypto_bignum_montinv_p384

        .private_extern _rscrypto_bignum_montinv_p384

        .text
        .balign 4
_rscrypto_bignum_montinv_p384:
        .cfi_startproc
        ...
        sub sp, sp, #(32*8 +0) %% .cfi_adjust_cfa_offset 32*8
...
Lbignum_montinv_p384_loop:
        ...
        ldr x6, [sp, #(16*8)+48]
        mov x7, #0xe000000000000000 %% adds x0, x0, x7 %% mov x8, #0x000000001fffffff %% adcs x1, x1, x8 %% mov x9, #0xffffffffe0000000 %% bic x9, x9, #0x2000000000000000 %% adcs x2, x2, x9 %% sbcs x3, x3, xzr %% sbcs x4, x4, xzr %% sbcs x5, x5, xzr %% mov x7, #0x1fffffffffffffff %% adc x6, x6, x7 %% add x0, x0, x0, lsl #32 %% mov x7, #0xffffffff00000001 %% umulh x7, x7, x0 %% mov x8, #0x00000000ffffffff %% mul x9, x8, x0 %% umulh x8, x8, x0 %% adds x7, x7, x9 %% adcs x8, x8, x0 %% cset x9, cs %% adds x6, x6, x0 %% cset x0, cs %% subs x1, x1, x7 %% sbcs x2, x2, x8 %% sbcs x3, x3, x9 %% sbcs x4, x4, xzr %% sbcs x5, x5, xzr %% sbcs x6, x6, xzr %% sbcs x0, x0, xzr %% neg x0, x0 %% and x7, x0, #0x00000000ffffffff %% and x8, x0, #0xffffffff00000000 %% and x9, x0, #0xfffffffffffffffe %% subs x1, x1, x7 %% sbcs x2, x2, x8 %% sbcs x3, x3, x9 %% sbcs x4, x4, x0 %% sbcs x5, x5, x0 %% sbc x6, x6, x0
        stp x1, x2, [sp, #(16*8)]
...
Lbignum_montinv_p384_midloop:
        ...
        ldr x5, [sp, #(16*8)+48]
        mov x7, #0xe000000000000000 %% adds x10, x10, x7 %% mov x8, #0x000000001fffffff %% adcs x0, x0, x8 %% mov x9, #0xffffffffe0000000 %% bic x9, x9, #0x2000000000000000 %% adcs x1, x1, x9 %% sbcs x2, x2, xzr %% sbcs x3, x3, xzr %% sbcs x4, x4, xzr %% mov x7, #0x1fffffffffffffff %% adc x5, x5, x7 %% add x10, x10, x10, lsl #32 %% mov x7, #0xffffffff00000001 %% umulh x7, x7, x10 %% mov x8, #0x00000000ffffffff %% mul x9, x8, x10 %% umulh x8, x8, x10 %% adds x7, x7, x9 %% adcs x8, x8, x10 %% cset x9, cs %% adds x5, x5, x10 %% cset x10, cs %% subs x0, x0, x7 %% sbcs x1, x1, x8 %% sbcs x2, x2, x9 %% sbcs x3, x3, xzr %% sbcs x4, x4, xzr %% sbcs x5, x5, xzr %% sbcs x10, x10, xzr %% neg x10, x10 %% and x7, x10, #0x00000000ffffffff %% and x8, x10, #0xffffffff00000000 %% and x9, x10, #0xfffffffffffffffe %% subs x0, x0, x7 %% sbcs x1, x1, x8 %% sbcs x2, x2, x9 %% sbcs x3, x3, x10 %% sbcs x4, x4, x10 %% sbc x5, x5, x10
        ...
        add sp, sp, #(32*8 +0) %% .cfi_adjust_cfa_offset -32*8
