...
pub(super) fn supports_bignum_mont_words(words: usize) -> bool {
  matches!(words, 16 | 24 | 32 | 48 | 64 | 128) && caps().has(x86::BMI2) && caps().has(x86::ADX)
}
...
pub(super) fn mont_mul_cios_words(
  ...
  // 3. The caller checks all slices have one of the supported RSA public or half-width CRT sizes:
  //    16, 24, 32, 48, 64, or 128 `u64` limbs. Every width is a multiple of four for the multiply
  //    kernel and of eight for the square kernel. The `t` capacity check preserves the shared RSA
  //    helper contract; this kernel uses stack scratch internally.
...
pub(super) fn mont_mul_cios_words_in_place_left(
  ...
  // 2. `supports_bignum_mont_words` checks BMI2 and ADX before this call, matching the kernel's
  //    `mulx/adcx/adox` instruction requirements.
  // 3. The caller checks `left`, `right`, and `modulus` are 16, 24, 32, 48, 64, or 128 `u64` limbs.
  //    Every width is a multiple of four for the multiply kernel. The kernel writes its result after
  //    consuming operands into internal stack scratch, so passing `left` as both `out` and `a`
  //    preserves the in-place helper contract.
