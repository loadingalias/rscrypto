use core::arch::x86_64::{
  __m128i, __m256i, __m512i, _mm256_add_epi16, _mm256_add_epi32, _mm256_and_si256, _mm256_cmpgt_epi16,
  _mm256_cmpgt_epi32, _mm256_cvtepi16_epi32, _mm256_mulhi_epi16, _mm256_mullo_epi16, _mm256_mullo_epi32,
  _mm256_or_si256, _mm256_permutevar8x32_epi32, _mm256_set_epi16, _mm256_set_epi32, _mm256_set1_epi16,
  _mm256_set1_epi32, _mm256_setzero_si256, _mm256_slli_epi32, _mm256_srai_epi32, _mm256_srli_epi32, _mm256_sub_epi16,
  _mm256_sub_epi32, _mm256_unpacklo_epi32, _mm512_add_epi32, _mm512_and_si512, _mm512_cmpgt_epi32_mask,
  _mm512_cvtepi16_epi32, _mm512_mask_add_epi32, _mm512_mask_sub_epi32, _mm512_mulhi_epi16, _mm512_mullo_epi16,
  _mm512_mullo_epi32, _mm512_or_si512, _mm512_set1_epi32, _mm512_setzero_si512, _mm512_slli_epi32, _mm512_srai_epi32,
  _mm512_srli_epi32, _mm512_sub_epi16,
};
...
/// # Safety
///
/// `ptr` must be valid to read 16 initialized `u16` values.
#[inline]
unsafe fn load_u16x16(ptr: *const u16) -> __m256i {
  // SAFETY: the caller provides 16 readable coefficients; `[u16; 16]` retains the pointer's two-byte alignment.
...
/// # Safety
///
/// `ptr` must be valid to write 16 `u16` values.
#[inline]
unsafe fn store_u16x16(ptr: *mut u16, value: __m256i) {
  // SAFETY: `[u16; 16]` and `__m256i` are 256-bit values, and every bit pattern is valid for both.
  let lanes = unsafe { core::mem::transmute::<__m256i, [u16; 16]>(value) };
  // SAFETY: the caller provides 16 writable coefficients; `[u16; 16]` retains the pointer's two-byte alignment.
  unsafe { ptr.cast::<[u16; 16]>().write_unaligned(lanes) };
}

/// Load 32 potentially unaligned coefficients without asserting SIMD alignment.
///
/// # Safety
///
/// `ptr` must be valid to read 32 initialized `u16` values.
#[inline]
unsafe fn load_u16x32(ptr: *const u16) -> __m512i {
  // SAFETY: the caller provides 32 readable coefficients; `[u16; 32]` retains the pointer's two-byte alignment.
  let lanes = unsafe { ptr.cast::<[u16; 32]>().read_unaligned() };
  // SAFETY: `[u16; 32]` and `__m512i` are 512-bit values, and every bit pattern is valid for both.
  unsafe { core::mem::transmute::<[u16; 32], __m512i>(lanes) }
}

/// Store 32 coefficients without asserting SIMD alignment.
///
/// # Safety
///
/// `ptr` must be valid to write 32 `u16` values.
#[inline]
unsafe fn store_u16x32(ptr: *mut u16, value: __m512i) {
  // SAFETY: `[u16; 32]` and `__m512i` are 512-bit values, and every bit pattern is valid for both.
  let lanes = unsafe { core::mem::transmute::<__m512i, [u16; 32]>(value) };
  // SAFETY: the caller provides 32 writable coefficients; `[u16; 32]` retains the pointer's two-byte alignment.
  unsafe { ptr.cast::<[u16; 32]>().write_unaligned(lanes) };
}

/// Load eight potentially unaligned signed coefficients without asserting SIMD alignment.
///
/// # Safety
///
/// `ptr` must be valid to read eight initialized `i16` values.
#[inline]
unsafe fn load_i16x8(ptr: *const i16) -> __m128i {
  // SAFETY: the caller provides eight readable coefficients; `[i16; 8]` retains the pointer's two-byte alignment.
  ...
  unsafe { core::mem::transmute::<[i16; 8], __m128i>(lanes) }
}

/// Load 16 potentially unaligned signed coefficients without asserting SIMD alignment.
///
/// # Safety
///
/// `ptr` must be valid to read 16 initialized `i16` values.
#[inline]
unsafe fn load_i16x16(ptr: *const i16) -> __m256i {
  // SAFETY: the caller provides 16 readable coefficients; `[i16; 16]` retains the pointer's two-byte alignment.
  let lanes = unsafe { ptr.cast::<[i16; 16]>().read_unaligned() };
  // SAFETY: `[i16; 16]` and `__m256i` are 256-bit values, and every bit pattern is valid for both.
  unsafe { core::mem::transmute::<[i16; 16], __m256i>(lanes) }
}
    ...
    while start < N {
      ...
      *zeta_index = (*zeta_index).strict_add(1);
      ...
      while j < end {
        ...
        unsafe {
          let u = load_u16x16(poly.as_ptr().add(j));
          let t = mul_mont_mod_u16x16_avx2(load_u16x16(poly.as_ptr().add(j.strict_add(len))), zeta);
          store_u16x16(poly.as_mut_ptr().add(j.strict_add(len)), sub_mod_u16x16_avx2(u, t));
          store_u16x16(poly.as_mut_ptr().add(j), add_mod_u16x16_avx2(u, t));
        }
        j = j.strict_add(16);
      }
    ...
    while start < N {
      ...
      *zeta_index = (*zeta_index).strict_sub(1);
      ...
      while j < end {
        ...
        //    AVX2 and SSE4.1 availability.
        unsafe {
          let t = load_u16x16(poly.as_ptr().add(j));
          let u = load_u16x16(poly.as_ptr().add(j.strict_add(len)));
          store_u16x16(poly.as_mut_ptr().add(j), add_mod_u16x16_avx2(t, u));
          store_u16x16(
...
pub(super) fn ntt_len2_avx2(poly: &mut Poly, zeta_index: &mut usize) {
  ...
  while start < N {
    ...
    *zeta_index = (*zeta_index).strict_add(4);
    ...
    //    AVX2 and SSE4.1 availability.
    unsafe {
      let values = load_u16x16(poly.as_ptr().add(start));
...
pub(super) fn multiply_ntts_accumulate_k3_avx2(acc: &mut Poly, a: [&Poly; 3], b: [&Poly; 3]) {
  for i in (0..GAMMAS_MONT.len()).step_by(8) {
    ...
    unsafe {
      ...
      let acc_pairs = load_u16x16(acc.as_ptr().add(coeff_offset));
...
/// # Safety
///
/// The active CPU must support AVX2, AVX-512F, AVX-512BW, AVX-512DQ, and SSE4.1.
pub(super) fn multiply_ntts_accumulate_k3_avx512(acc: &mut Poly, a: [&Poly; 3], b: [&Poly; 3]) {
  for i in (0..GAMMAS_MONT.len()).step_by(16) {
    ...
    unsafe {
      ...
      let acc_pairs = load_u16x32(acc.as_ptr().add(coeff_offset));
...
fn montgomery_reduce_s16x16_avx2(low: __m256i, high: __m256i) -> __m256i {
  let k = _mm256_mullo_epi16(low, _mm256_set1_epi16(Q_MONT_INV_U16.cast_signed()));
...
fn base_multiply_chunk_avx2(a: &Poly, b: &Poly, gamma: __m256i, coeff_offset: usize) -> (__m256i, __m256i) {
  ...
  unsafe {
    let a_pairs = load_u16x16(a.as_ptr().add(coeff_offset));
    let b_pairs = load_u16x16(b.as_ptr().add(coeff_offset));
...
fn store_u32_pair_lanes_as_u16_avx2(ptr: *mut u16, lo: __m256i, hi: __m256i) {
  ...
  unsafe { store_u16x16(ptr, packed) };
}
...
fn load_i16x8_as_i32x8_avx2(ptr: *const i16) -> __m256i {
  ...
  let packed = unsafe { load_i16x8(ptr) };
...
fn add_mod_u32x8_avx2(a: __m256i, b: __m256i) -> __m256i {
  ...
  let ge_q = _mm256_cmpgt_epi32(sum, _mm256_set1_epi32((Q_U32 - 1).cast_signed()));
...
/// # Safety
///
/// The active CPU must support AVX2, AVX-512F, AVX-512BW, AVX-512DQ, and SSE4.1, and
/// `coeff_offset..coeff_offset + 32` must be in bounds for both polynomials.
fn base_multiply_chunk_avx512(a: &Poly, b: &Poly, gamma: __m512i, coeff_offset: usize) -> (__m512i, __m512i) {
  ...
  unsafe {
    let a_pairs = load_u16x32(a.as_ptr().add(coeff_offset));
    let b_pairs = load_u16x32(b.as_ptr().add(coeff_offset));
...
fn store_u32_pair_lanes_as_u16_avx512(ptr: *mut u16, lo: __m512i, hi: __m512i) {
  ...
  unsafe { store_u16x32(ptr, packed) };
}
...
fn load_i16x16_as_i32x16_avx512(ptr: *const i16) -> __m512i {
  ...
  let packed = unsafe { load_i16x16(ptr) };
...
fn add_mod_u32x16_avx512(a: __m512i, b: __m512i) -> __m512i {
  ...
  let ge_q = _mm512_cmpgt_epi32_mask(sum, _mm512_set1_epi32((Q_U32 - 1).cast_signed()));
