...
impl Aead for Aes256Gcm {
  ...
  fn __encrypt_in_place_with_nonce(
    ...
    if self.backend == AeadBackend::X86VaesVpclmul && buffer.len() >= X86_VAES_GCM_MIN_LEN {
      ...
      acc.0 = unsafe { aes::aes256_ctr32_encrypt_be_wide_ghash(&self.ek, &ctr_block, buffer, acc.0, tables) };
      acc.0 ^= u128::from_be_bytes(length_block);
    ...
    if matches!(
      ...
      acc.0 = unsafe { aes::aes256_ctr32_encrypt_be_aarch64_ghash(&self.ek, &ctr_block, buffer, acc.0, &tables) };
      acc.0 ^= u128::from_be_bytes(length_block);
    ...
    if self.backend == AeadBackend::Power8Crypto {
      ...
      acc.0 = unsafe {
        aes::aes256_ctr32_encrypt_be_ppc_ghash(&self.ek, &ctr_block, buffer, acc.0, h_polyval, &self.h_powers_rev)
      };
      acc.0 ^= u128::from_be_bytes(length_block);
  ...
  fn decrypt_in_place(
    ...
    if self.backend == AeadBackend::X86VaesVpclmul && buffer.len() >= X86_VAES_GCM_MIN_LEN {
      ...
      acc.0 = unsafe { aes::aes256_ctr32_decrypt_be_wide_ghash(&self.ek, &ctr_block, buffer, acc.0, tables) };
      acc.0 ^= u128::from_be_bytes(length_block);
      // SAFETY: x86 GHASH final multiply because:
      // 1. Backend resolution selected X86VaesVpclmul only after CPUID confirmed PCLMULQDQ.
      // 2. `acc` and `h_polyval` are initialized GHASH field elements.
    ...
    if matches!(
      ...
      acc.0 = unsafe { aes::aes256_ctr32_decrypt_be_aarch64_ghash(&self.ek, &ctr_block, buffer, acc.0, &tables) };
      acc.0 ^= u128::from_be_bytes(length_block);
      // SAFETY: aarch64 GHASH final multiply because:
      // 1. Backend resolution selected an AES/PMULL backend only after runtime detection confirmed PMULL.
      // 2. `acc` and `h_polyval` are initialized GHASH field elements.
    ...
    if self.backend == AeadBackend::Power8Crypto {
      ...
      acc.0 = unsafe {
        aes::aes256_ctr32_decrypt_be_ppc_ghash(&self.ek, &ctr_block, buffer, acc.0, h_polyval, &self.h_powers_rev)
      };
      acc.0 ^= u128::from_be_bytes(length_block);
      // SAFETY: POWER8 carryless multiply because backend resolution confirmed POWER8 crypto.
      acc.0 = unsafe { polyval::ppc_clmul128_reduce_inline(acc.0, h_polyval) };
