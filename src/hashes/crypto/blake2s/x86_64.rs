...
#[inline(always)]
fn ror16_avx2(x: __m128i) -> __m128i {
...
#[inline(always)]
fn ror12_avx2(x: __m128i) -> __m128i {
  // SAFETY: SSE2 intrinsics operate only on the provided SIMD register.
  unsafe { _mm_or_si128(_mm_srli_epi32(x, 12), _mm_slli_epi32(x, 20)) }
}

#[inline(always)]
fn ror8_avx2(x: __m128i) -> __m128i {
  // SAFETY: SSE2 intrinsics operate only on the provided SIMD register.
  unsafe { _mm_or_si128(_mm_srli_epi32(x, 8), _mm_slli_epi32(x, 24)) }
}

#[inline(always)]
fn ror7_avx2(x: __m128i) -> __m128i {
  // SAFETY: SSE2 intrinsics operate only on the provided SIMD register.
  unsafe { _mm_or_si128(_mm_srli_epi32(x, 7), _mm_slli_epi32(x, 25)) }
}

#[inline(always)]
fn g_avx2(a: &mut __m128i, b: &mut __m128i, c: &mut __m128i, d: &mut __m128i, mx: __m128i, my: __m128i) {
...
#[inline(always)]
fn diagonalize(b: &mut __m128i, c: &mut __m128i, d: &mut __m128i) {
...
#[inline(always)]
fn undiagonalize(b: &mut __m128i, c: &mut __m128i, d: &mut __m128i) {
  // SAFETY: shuffle permutes only the provided SIMD registers.
  unsafe {
...
#[inline(always)]
fn load_msg_quad(m: &[u32; 16], i0: u8, i1: u8, i2: u8, i3: u8) -> __m128i {
  // SAFETY: `_mm_set_epi32` constructs a register from in-bounds message words.
  unsafe {
    _mm_set_epi32(
      m[i3 as usize].cast_signed(),
      m[i2 as usize].cast_signed(),
      m[i1 as usize].cast_signed(),
      m[i0 as usize].cast_signed(),
    )
  }
}
...
#[inline(always)]
/// Applies one BLAKE2s mixing step with AVX-512VL rotates.
///
/// # Safety
///
/// The caller must execute this function only when AVX-512F and AVX-512VL are
/// enabled for the current execution scope.
