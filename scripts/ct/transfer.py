"""Transfer RISC-V CT preparation without rebuilding the measured executable."""
...
KIND = 'rscrypto.riscv.ct'
GATES = ('ct-artifacts', 'ct-validate-artifacts', 'ct-zeroization-sentinel')


def export(root, out_dir, shared, steps, identity, archive):
...
def consume(root, out_dir, archive):
    if platform.system() != 'Linux' or platform.machine() != 'riscv64':
        raise ValueError('transferred CT measurements require physical RISC-V Linux')
