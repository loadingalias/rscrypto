...
# Used by constant-time artifact and BINSEC cross-linking.
...
#
# Usage (automatic via the CC/linker environment in CT scripts):
...
riscv64gc-unknown-linux-gnu | riscv64-unknown-linux-gnu) ZIG_TARGET="riscv64-linux-gnu" ;;
...
*)
	...
	echo "Add mapping to scripts/ct/zig-cc.sh if this is a valid target" >&2
	exit 1
	;;
esac
