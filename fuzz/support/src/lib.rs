...
impl<'a> FuzzInput<'a> {
  ...
  pub fn bytes<const N: usize>(&mut self) -> Option<[u8; N]> {
    ...
    let arr = self.data[self.pos..self.pos.strict_add(N)].try_into().ok()?;
...
pub fn assert_aead_roundtrip<A: Aead>(cipher: &A, nonce: &A::Nonce, aad: &[u8], plaintext: &[u8]) {
  ...
  assert_eq!(recovered, original, "roundtrip: combined encrypt/decrypt mismatch");
}
...
/// 2. Oracle encrypts; rscrypto decrypts after splitting the `A::TAG_SIZE` suffix from the body.
///
...
pub fn assert_mac_against_oracle<M: Mac>(
  ...
  assert_eq!(tag.as_ref(), oracle_tag.as_slice(), "mac: oracle tag mismatch");
}
...
pub fn assert_hkdf_against_oracle(ours_ok: bool, ours_okm: &[u8], oracle_ok: bool, oracle_okm: &[u8]) {
