...
//
// 1. No-panic on adversarial input. `decode_string` for every supported algorithm must return a
//    `Result`, never panic, regardless of how structurally invalid the candidate string is.
// 2. Roundtrip on freshly-emitted strings. A variant byte selects Argon2id/2d/2i/Scrypt;
//    verify_string must accept the fresh emission, and any single-byte XOR mutation must produce
//    verify_string-Err.
//
...
use rscrypto::{Argon2Params, Argon2Version, Argon2d, Argon2i, Argon2id, Scrypt, ScryptParams};
...
fn assert_roundtrip(encoded: &str, password: &[u8], variant: u8) {
  match variant % 4 {
    0 => Argon2id::verify_string(password, encoded).expect("argon2id verify_string must accept its own emission"),
    1 => Argon2d::verify_string(password, encoded).expect("argon2d verify_string must accept its own emission"),
    2 => Argon2i::verify_string(password, encoded).expect("argon2i verify_string must accept its own emission"),
    _ => Scrypt::verify_string(password, encoded).expect("scrypt verify_string must accept its own emission"),
  }
}
...
fn assert_byte_flip_rejected(encoded: &str, password: &[u8], variant: u8, flip_byte: u8) {
  ...
  bytes[idx] ^= 0x01;
  let mutated = core::str::from_utf8(&bytes).expect("ASCII PHC string XOR-1 must remain valid UTF-8");
  ...
  assert!(result.is_err(), "verify_string must reject single-bit XOR mutation");
}
