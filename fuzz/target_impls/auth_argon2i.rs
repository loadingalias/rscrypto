...
pub fn run(data: &[u8]) {
  ...
  Argon2i::derive(&params, password, &salt_buf, &mut actual).expect("hash within fuzz ranges");

  Argon2i::verify(&params, password, &salt_buf, &actual).expect("verify must accept the hash it just produced");
