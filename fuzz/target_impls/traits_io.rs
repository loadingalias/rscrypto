...
use rscrypto_fuzz::{FuzzInput, PartialReader, PartialWriter, some_or_return, split_at_ratio};
...
fn fuzz_digest_reader(data: &[u8], ops: &[u8], max_per_call: usize) {
  ...
  assert_eq!(reader.digest(), Blake3::digest(&seen), "digest reader prelude mismatch");

  for &op in ops.iter().take(32) {
    reader_step(&mut reader, &mut seen, op).expect("digest reader step");
    assert_eq!(reader.digest(), Blake3::digest(&seen), "digest reader step mismatch");
  }
  ...
  loop {
    ...
    seen.extend_from_slice(&buf[..n]);
    assert_eq!(reader.digest(), Blake3::digest(&seen), "digest reader drain mismatch");
    if n == 0 {
      break;
    }
  }
