use rscrypto::{
  Kem as _, MlKem1024, MlKem1024Ciphertext, MlKem1024DecapsulationKey, MlKem1024EncapsulationKey, MlKemError,
};
...
pub fn run(data: &[u8]) {
  ...
  let (ek, dk) = MlKem1024::generate_keypair(|out| {
    out.copy_from_slice(&key_random);
    Ok::<(), MlKemError>(())
  })
  .expect("fixed-size ML-KEM keygen randomness must be accepted");
  let (ciphertext, encapsulated) = MlKem1024::encapsulate(&ek, |out| {
