...
fn assert_monotonic_and_roundtrip(
  ...
  for i in 0..burst {
    let nonce = counter
      .next_nonce()
      .expect("counter must not exhaust within bounded burst");
  ...
  assert_eq!(
    counter.issued().strict_add(counter.remaining()),
...
fn assert_resume_equivalence(prefix: [u8; 4], advance: u32) {
  ...
  let mut resumed =
    NonceCounter::<Aes256Gcm>::with_counter(prefix, u64::from(advance)).expect("resume below cap must succeed");
...
pub fn run(data: &[u8]) {
  ...
  let mut counter =
    NonceCounter::<Aes256Gcm>::with_counter(prefix, initial).expect("initial < MAX_MESSAGES by construction");
