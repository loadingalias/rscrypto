...
fn tune_algorithm_impl(
  ...
  // BLAKE3: tune automatic multi-core policy cutoffs and runtime cost-model
  // terms for one-shot and streaming workloads separately.
  if (algorithm.name() == "blake3" || algorithm.name().starts_with("blake3-stream4k"))
    && let Some(policy) = tune_blake3_parallel_policy(runner, algorithm, best_kernel)?
  {
...
fn blake3_policy_from_curves(
  ...
  let (spawn_cost_bytes, merge_cost_bytes) = if let Some((slope, intercept)) = fit_line_thread_bytes(&crossover_samples)
  {
    ...
    // Keep spawn overhead stable and monotonic; oversized values over-throttle.
