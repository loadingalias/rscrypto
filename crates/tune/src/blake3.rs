//! BLAKE3 tunable implementation.

use alloc::{string::String, vec, vec::Vec};

use hashes::bench::{self, Blake3Kernel};
use platform::Caps;
...
const BLAKE3_PARAMS: &[TunableParam] = &[];

fn blake3_kernel_specs(_caps: &Caps) -> Vec<KernelSpec> {
  vec![KernelSpec::new("portable", KernelTier::Portable, Caps::NONE)]
}

/// Tunable implementation for BLAKE3.
pub struct Blake3Tunable {
  forced_kernel: Option<String>,
  cached_kernel: Option<Blake3Kernel>,
...
impl Blake3Tunable {
  ...
  fn resolve_kernel(&mut self) {
    if let Some(ref name) = self.forced_kernel {
      self.cached_kernel = bench::get_blake3_kernel(name);
...
impl Default for Blake3Tunable {
  fn default() -> Self {
    Self::new()
  }
}

impl crate::Tunable for Blake3Tunable {
  fn name(&self) -> &'static str {
    "blake3"
  }

  fn available_kernels(&self, caps: &Caps) -> Vec<KernelSpec> {
    blake3_kernel_specs(caps)
  }
  ...
  fn benchmark(&self, data: &[u8], _iterations: usize) -> BenchResult {
    ...
    let (kernel_name, result) = if let Some(kernel) = self.cached_kernel {
      ...
      let result = sampler.run(data, |buf| {
        core::hint::black_box(hashes::crypto::blake3::dispatch::digest(core::hint::black_box(buf)));
      });
      (hashes::crypto::blake3::dispatch::kernel_name_for_len(data.len()), result)
    };
