...
//! - CRC-32: slice-by-4, slice-by-8, slice-by-16
...
//! | 32-bit | slice-by-4 | 4 | 4×256×u32 | ~1.5 GB/s |
//! | 32-bit | slice-by-8 | 8 | 8×256×u32 | ~2.5 GB/s |
//! | 32-bit | slice-by-16 | 16 | 16×256×u32 | ~3.5 GB/s |
...
/// Update CRC-32 state using slice-by-4 algorithm.
///
/// Processes 4 bytes per iteration (1× the CRC width in bytes).
///
/// # Arguments
///
/// * `crc` - Current CRC state (pre-inverted if applicable)
...
#[inline]
#[allow(dead_code)]
pub fn slice4_32(mut crc: u32, data: &[u8], tables: &[[u32; 256]; 4]) -> u32 {
  let (chunks, remainder) = data.as_chunks::<4>();
...
pub fn slice8_32(mut crc: u32, data: &[u8], tables: &[[u32; 256]; 8]) -> u32 {
  ...
  for chunk in chunks {
    // Read 8 bytes as two u32 values
...
pub fn slice16_32(mut crc: u32, data: &[u8], tables: &[[u32; 256]; 16]) -> u32 {
  ...
  // Handle remainder chunks (0-3 u32s = 0-12 bytes from 16-byte alignment)
  ...
  // Process remaining bytes (0-3) with byte-at-a-time
  for &byte in remainder {
    let index = ((crc ^ (byte as u32)) & 0xFF) as usize;
    crc = tables[0][index] ^ (crc >> 8);
  }
...
mod tests {
  ...
  /// Generate CRC-32 tables for testing (IEEE polynomial 0xEDB88320 reflected).
  ...
  const fn generate_crc32_tables_4(poly: u32) -> [[u32; 256]; 4] {
  ...
  const fn generate_crc32_tables_8(poly: u32) -> [[u32; 256]; 8] {
  ...
  const fn generate_crc32_tables_16(poly: u32) -> [[u32; 256]; 16] {
  ...
  const CRC32_IEEE_POLY: u32 = 0xEDB8_8320; // Reflected

  #[test]
  fn test_slice4_32_empty() {
    let tables = generate_crc32_tables_4(CRC32_IEEE_POLY);
    assert_eq!(slice4_32(!0, &[], &tables), !0);
  }

  #[test]
  fn test_slice8_32_empty() {
    let tables = generate_crc32_tables_8(CRC32_IEEE_POLY);
    assert_eq!(slice8_32(!0, &[], &tables), !0);
  }

  #[test]
  fn test_slice16_32_empty() {
    let tables = generate_crc32_tables_16(CRC32_IEEE_POLY);
    assert_eq!(slice16_32(!0, &[], &tables), !0);
  }

  #[test]
  fn test_slice4_32_matches_slice8_32() {
    let tables4 = generate_crc32_tables_4(CRC32_IEEE_POLY);
    let tables8 = generate_crc32_tables_8(CRC32_IEEE_POLY);
    let data = b"The quick brown fox jumps over the lazy dog";

    let a = slice4_32(!0, data, &tables4);
    let b = slice8_32(!0, data, &tables8);
    assert_eq!(a, b);
  }

  #[test]
  fn test_slice8_32_matches_slice16_32() {
    let tables8 = generate_crc32_tables_8(CRC32_IEEE_POLY);
    let tables16 = generate_crc32_tables_16(CRC32_IEEE_POLY);
    let data = b"The quick brown fox jumps over the lazy dog";

    let a = slice8_32(!0, data, &tables8);
    let b = slice16_32(!0, data, &tables16);
    assert_eq!(a, b);
  }

  #[test]
  fn test_slice16_32_incremental() {
    let tables = generate_crc32_tables_16(CRC32_IEEE_POLY);
    let data = b"hello world, this is a longer test string for crc32";
    let full = slice16_32(!0, data, &tables);

    for split in [1, 3, 4, 7, 8, 15, 16, 17, 20, 32] {
      if split < data.len() {
        let crc1 = slice16_32(!0, &data[..split], &tables);
        let crc2 = slice16_32(crc1, &data[split..], &tables);
        assert_eq!(crc2, full, "Incremental failed at split {split}");
      }
    }
  }
  ...
  fn test_crc32_ieee_test_vector() {
    // "123456789" should produce 0xCBF43926 for CRC-32 (IEEE)
    let tables = generate_crc32_tables_8(CRC32_IEEE_POLY);
    let crc = slice8_32(!0, b"123456789", &tables) ^ !0;
