...
pub const CRC24_TUNED_DEFAULTS: &[(TuneKind, Crc24TunedDefaults)] = &[
  // BEGIN GENERATED (rscrypto-tune)  // Default: conservative x86_64 tuning (used when microarch is unknown).  (TuneKind::Default, Crc24TunedDefaults { slice4_to_slice8: 64, portable_to_clmul: 64, pclmul_to_vpclmul: None, streams: 2, min_bytes_per_lane: Some(256) }),  // Zen4: VPCLMUL is fastest; use it immediately. Multi-stream only pays off at very large sizes.
