...
//!
//! CRC32 uses polynomial 0x04C11DB7 (reflected: 0xEDB88320), the ubiquitous
//! "Ethernet/zlib/gzip/PNG/zip" CRC.
//!
//! # Performance
//!
...
//!
//! - **aarch64**: PMULL+EOR3 → PMULL → portable
//! - **x86_64**: PCLMULQDQ → portable (no SSE4.2 CRC32 for ISO polynomial)
...
/// CRC32 (ISO-HDLC) checksum.
///
/// This struct implements streaming CRC32 computation with automatic
/// hardware acceleration when available.
///
/// # Thread Safety
///
/// `Crc32` is `Send` and `Sync`. Multiple hashers can operate in parallel
...
impl Crc32 {
  /// Initial value for CRC32 (all ones).
  ...
  /// Compute CRC32 of data in one shot.
  ///
  ...
  /// Current checksum value (alias for [`finalize`](Self::finalize)).
  #[inline]
  #[must_use]
  ...
  /// Combine two CRC32 values: `crc(A || B)` from `crc(A)`, `crc(B)`, `len(B)`.
  ///
  /// This enables parallel CRC computation in O(log n) time.
  #[inline]
  #[must_use]
  pub fn combine(crc_a: u32, crc_b: u32, len_b: usize) -> u32 {
    crate::combine::crc32_combine(crc_a, crc_b, len_b)
  }
}
...
/// Returns the CRC32 backend this build will use on the current machine.
#[doc(hidden)]
#[inline]
#[must_use]
#[cfg(all(
...
fn dispatch(crc: u32, data: &[u8]) -> u32 {
  ...
  ))]
  {
    ...
    crate::simd::aarch64::pmull::compute_pmull_eor3_crc32_enabled(crc, data)
  }
...
mod tests {
  ...
  fn test_simd_matches_portable_aarch64() {
    ...
    for &len in &lengths {
      let data = gen_bytes(len, len as u64 ^ 0x5D58_39A7_3D87_1CEBu64);
      for &init in &inits {
        ...
        if std::arch::is_aarch64_feature_detected!("aes") && std::arch::is_aarch64_feature_detected!("crc") {
          ...
          {
            let got = crate::simd::aarch64::pmull::compute_pmull_eor3_crc32_enabled(init, &data);
          ...
          #[cfg(not(all(target_feature = "aes", target_feature = "crc", target_feature = "sha3")))]
          {
            if std::arch::is_aarch64_feature_detected!("sha3") {
              let got = crate::simd::aarch64::pmull::compute_pmull_eor3_crc32_runtime(init, &data);
              assert_eq!(got, expected, "pmull+eor3 mismatch at len={}", len);
            } else {
              #[cfg(all(target_feature = "aes", target_feature = "crc"))]
              let got = crate::simd::aarch64::pmull::compute_pmull_crc32_enabled(init, &data);
              #[cfg(not(all(target_feature = "aes", target_feature = "crc")))]
              let got = crate::simd::aarch64::pmull::compute_pmull_crc32_runtime(init, &data);
