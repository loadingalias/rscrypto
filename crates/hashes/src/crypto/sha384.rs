...
// SHA-384 initial hash value (FIPS 180-4).
const H0: [u64; 8] = [
...
// SHA-512 K constants (shared by SHA-384).
const K: [u64; 80] = [
...
impl Default for Sha384 {
  ...
  fn default() -> Self {
    Self {
      ...
      block_len: 0,
      bytes_hashed: 0,
      compress_blocks: Sha384::compress_blocks_portable,
      dispatch: None,
    }
  }
}
...
impl Sha384 {
  ...
  fn compress_block(state: &mut [u64; 8], block: &[u8; BLOCK_LEN]) {
    // 16-word ring buffer message schedule.
    let mut w = [0u64; 16];
