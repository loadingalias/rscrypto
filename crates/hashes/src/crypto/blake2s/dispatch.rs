...
use super::{
  BLOCK_LEN, Blake2s256,
  dispatch_tables::DispatchTable,
  kernels::{Blake2s256KernelId, CompressFn, compress_fn, required_caps},
};
...
#[inline]
#[must_use]
fn resolve(id: Blake2s256KernelId, caps: Caps) -> Blake2s256KernelId {
  if caps.has(required_caps(id)) {
    id
  } else {
    Blake2s256KernelId::Portable
  }
}
...
#[inline]
pub fn compress(h: &mut [u32; 8], blocks: &[u8], bytes_hashed: &mut u64, is_last: bool, last_block_len: u32) {
  if blocks.is_empty() {
    return;
  }
