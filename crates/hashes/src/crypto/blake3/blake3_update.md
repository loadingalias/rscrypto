...
### Tomorrow Plan (2026-02-23)

#### Honest status
- Yes, there is still a path to close Phases 3-5, but **not** with more micro-indirection tweaks.
- Current evidence says the remaining short-input deficit is mostly compute-path competitiveness on x86 (`intel-spr`, `intel-icl`) and still significant on aarch64 (`graviton3/4`).
- If we keep trying tiny structural edits, probability of closing gates is low.
- If we switch to targeted kernel-policy + compute-path experiments with strict accept/reject criteria, probability is moderate.

#### Probability (based on current data)
- Close Phase 3 (short-path wins with stable reruns): **~70%**
- Close Phase 4 (lane-specific tightening good enough for all enforced lanes): **~45%**
- Close Phase 5 (lock-in, stable gates, no regressions): **~30%**

#### Non-negotiable constraints
- Pure Rust only, no C libs, no new deps (except existing Rayon policy).
- Keep API simple and uniform.
- No regressions at `4096+`.
- `ROOT` only on final block/root formation.

#### Execution order for tomorrow
1. Reconfirm baseline (`Candidate I`) on x86 apples and full attribution.
2. Run forced-kernel apples split on x86 (`x86_64/avx2` and `x86_64/avx512`) to isolate policy-vs-compute for `256/1024`.
3. Pick one x86 compute candidate that changes real work (not dispatch plumbing), then rerun apples twice.
4. If x86 candidate is stable-positive, run full-lane attribution.
5. Only after full-lane positive signal, run gate workflows.

#### Go/No-Go rules
- Promote only if both x86 control lanes (`amd-zen4`, `intel-spr`) improve or hold within noise at **both** `256` and `1024`.
- Reject immediately if one lane improves while the other regresses materially.
- Reject any candidate that improves apples but worsens full-lane attribution on `graviton3/4`.
- Do not tune gate thresholds until repeated pass signal exists.

### Next Session Execution Packet (2026-02-23)

#### Locked baseline to compare against
- Code baseline: `main` at `3f6ae9a` (Candidate I active; Candidate J reverted; Candidate K reverted).
- Apples baseline run: `22269244541` (`oneshot-apples`, lanes `amd-zen4`, `intel-spr`).
- Full-lane attribution baseline run: `22268280116` (all enforced lanes).

#### Required run order (do not skip)
1. Re-run apples baseline for noise window:
   - same filter/lane set as `22269244541`.
2. Forced-kernel apples split on x86 controls:
   - `blake3_force_kernel=x86_64/avx2`
   - `blake3_force_kernel=x86_64/avx512`
   - lanes: `amd-zen4`, `intel-spr`.
3. Build exactly one compute-path candidate (no dispatch-table edits), run apples twice.
4. If and only if apples are positive/neutral on both lanes at `256/1024`, run full-lane attribution.
5. Gate workflows only after full-lane attribution is positive.

#### Candidate acceptance thresholds
- `oneshot-apples`:
  - required: no material regression on either x86 control lane at `256` or `1024`;
  - preferred: improvement on `intel-spr` plain API at both sizes.
- `short-input-attribution` full lane:
  - required: no regressions on `graviton3/4` short gaps,
  - required: no regression at `4096+` across enforced lanes.
- Promotion policy:
  - promote only after one stability rerun confirms signal direction.

#### Explicit anti-patterns (ban list)
- No more micro-indirection-only candidates (already exhausted by Candidate J).
- No `ROOT` flag usage in non-final chunk compression paths.
- No architecture-specific fallbacks unless a single-lane regression is both repeatable and isolated.

### Phase 4 Candidate L (local-only, pending CI validation, 2026-02-22)
- Implemented on local workspace:
  - added a block-aligned short-chunk fast path in `ChunkState::update()` for `128/256/512/1024` byte updates when entering a fresh chunk:
    - compresses prefix (`len - 64`) in one call,
    - buffers exactly one final full block (`64`) for `CHUNK_END` handling at finalize.
  - scope is limited to single-chunk, block-aligned updates; no dispatch-table edits and no large-input tree-path edits.
- Rationale:
  - this removes loop/branch overhead in the hottest API path shape (`new + update + finalize`) for the exact short lengths driving gate failures.
  - unlike Candidate J, this changes work structure in the streaming compute path, not just a call-indirection detail.
- Local verification:
  - `cargo fmt --all --check`: pass
  - `cargo test -p hashes blake3:: --quiet`: pass
- Next required measurement:
  - x86 apples first:
    - `Bench` workflow, `crates=hashes`, `benches=blake3_short_input_attribution`, `filter=oneshot-apples`, `quick=false`, lanes `amd-zen4,intel-spr`.
  - stability rerun on the same x86 apples matrix.
  - if positive/neutral, run full-lane attribution:
    - `filter=short-path-split,oneshot-apples,oneshot-dispatch-overhead,stream-dispatch-overhead`
    - lanes: all enforced.

#### Candidate L CI command packet
1. x86 apples:
   - `scripts/bench/bench.sh crates=hashes benches=blake3_short_input_attribution filter=oneshot-apples quick=false platforms=amd-zen4,intel-spr`
2. x86 apples rerun:
   - same command as step 1.
3. full-lane attribution:
   - `scripts/bench/bench.sh crates=hashes benches=blake3_short_input_attribution filter=short-path-split,oneshot-apples,oneshot-dispatch-overhead,stream-dispatch-overhead quick=false`
