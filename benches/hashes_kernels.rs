...
fn print_blake3_dispatch_info(oneshot_lens: impl Iterator<Item = usize>) {
  ...
  let (stream, bulk) = rscrypto::hashes::crypto::blake3::dispatch::streaming_kernel_names();
  ...
  for len in oneshot_lens {
    let kernel = rscrypto::hashes::crypto::blake3::dispatch::kernel_name_for_len(len);
