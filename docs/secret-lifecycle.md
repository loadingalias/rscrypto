...
| ECDSA, X25519, ML-KEM, and RSA private operations                        | Secret scalars, decapsulation arithmetic, private components, caller-random salt and blinding values, 64-bit limbs, 31-bit batched-inverse words, encoded-message buffers, and reusable private scratch | Typed and nested owner `Drop`; operation wrappers clear local arrays and initialized heap regions; the RSA inverse clears all GCD/coefficient/modulus words after success or failure; rejected private outputs are cleared |
