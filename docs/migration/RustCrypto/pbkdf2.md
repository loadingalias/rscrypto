...
Verified against `pbkdf2 = "0.13.0"` and the `rscrypto` 0.1 line.

## TL;DR

| | Before (`pbkdf2` 0.13.x) | After (`rscrypto` 0.1) |
|---|---|---|
| Cargo dep | `pbkdf2 = "0.13"` + `sha2 = "0.11"` | `rscrypto = { version = "0.1", features = ["pbkdf2"] }` |
...
# After
[dependencies]
rscrypto = { version = "0.1", features = ["pbkdf2"] }
```
