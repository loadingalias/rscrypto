...
Verified against `sha3 = "0.11.0"` and the `rscrypto` 0.1 line.

## TL;DR

| | Before (`sha3` 0.11.x) | After (`rscrypto` 0.1) |
|---|---|---|
| Cargo dep | `sha3 = "0.11"` | `rscrypto = { version = "0.1", features = ["sha3"] }` |
...
# After
[dependencies]
rscrypto = { version = "0.1", features = ["sha3"] }
```
