...
- **`no_std`.** `crc32fast` requires `std` for runtime SIMD detection. rscrypto detects at runtime when `std` is enabled and falls back to compile-time `target_feature` selection in `no_std` builds. The `portable-only` feature pins to the audited portable kernel for FIPS / DO-178C / IEC 62443 lanes.
