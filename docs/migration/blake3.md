...
- **`no_std`.** Both crates support `no_std` with portable fallbacks. rscrypto
  runtime-detects SIMD when `std` is enabled. `portable-only` makes runtime
  capability detection ignore host acceleration; see
