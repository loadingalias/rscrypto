...
Verified against `rapidhash = "4.4.1"` (V3 with avalanche, default secrets) and the `rscrypto` 0.1 line.

## TL;DR

| | Before (`rapidhash` 4.x) | After (`rscrypto` 0.1) |
|---|---|---|
| Cargo dep | `rapidhash = "4.4"` | `rscrypto = { version = "0.1", features = ["rapidhash"] }` |
...
# After
[dependencies]
rscrypto = { version = "0.1", features = ["rapidhash"] }
```
